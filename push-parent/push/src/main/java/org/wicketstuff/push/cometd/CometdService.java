...
public class CometdService implements IChannelService {
  ...
  private final class RemovalForwardingListener implements
    ...
    public void deliver(final Client fromClient, final Client toClient,
      ...
      if (removalListeners.containsKey(channel)) {
        fromClient.addListener((ClientListener) removalListeners.get(channel));
      }
    }
  }

  private final class RemovalListenerWrapper implements RemoveListener {
    private final WeakReference _clientSession;
    private final RemoveListener _listener;

    public RemovalListenerWrapper(final Session clientSession, final RemoveListener listener) {
      _clientSession = new WeakReference(clientSession);
      _listener = listener;
    }

    public void removed(final String arg0, final boolean arg1) {
      if (!Application.exists()) {
        Application.set(_application);
      }
      final Session oldSess = (Session) _clientSession.get();
      if (!Session.exists() && oldSess != null) {
        Session.set(oldSess);
      }
      _listener.removed(arg0, arg1);
    }
  ...
  public void addChannelRemoveListener(final String chnl,
    ...
    removalListeners.put("/" + chnl, new RemovalListenerWrapper(Session.get(),
        listener));
  }

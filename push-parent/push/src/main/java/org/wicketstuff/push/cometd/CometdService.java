...
public class CometdService implements IChannelService {
  ...
  private final class RemovalForwardingListener implements MessageListener {
	...
	public boolean onMessage(final ServerSession fromClient, final ServerSession toClient, final ServerMessage msg) {
...
//      getBayeux().getChannel(Channel.META_SUBSCRIBE).subscribe(serviceClient);
    	Object serverChannel = getBayeux().getChannel(Channel.META_SUBSCRIBE);
...
//    final ClientSessionChannel channel = (ClientSessionChannel) getBayeux().getSession("").getLocalSession().g Channel(channelId);

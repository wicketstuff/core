...
public class HideWhenEmptyOrNullBehaviorTest
{
	...
	{
		tester.startComponentInPage(
			new Label(CID_LABEL, new ListModel<String>(Collections.<String>emptyList())).add(HideWhenEmptyOrNullBehavior.get()),
			Markup.of(MARKUP));
		Assert.assertNull(tester.getTagByWicketId(CID_LABEL));
	}
	...
	{
		tester.startComponentInPage(
			new Label(CID_LABEL, new ListModel<String>(Arrays.asList("test"))).add(HideWhenEmptyOrNullBehavior.get()),
			Markup.of(MARKUP));
		Assert.assertNotNull(tester.getTagByWicketId(CID_LABEL));
	}

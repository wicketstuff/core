...
public interface IRenderStrategy
{
	...
	{
		public void mapEvent(Map<DateMidnight, List<IEvent>> map, IEvent event,
	...
	public static final IRenderStrategy EVERY_DAY_OF_EVENT = new IRenderStrategy()
	{
			...
			BaseCalendarView calendar)
		{
			...
			if (event.getEndTime() != null &&
				event.getEndTime().equals(event.getStartTime()) == false)
			{
				end = new DateMidnight(event.getEndTime());
			}
			if (end.isAfter(start))
			{
				for (Iterator<DateMidnight> it = new DateMidnightIterator(start.toDateTime(),
					end.toDateTime()); it.hasNext();)
				{
	...
	{
		private static void addEventToDate(Map<DateMidnight, List<IEvent>> map, DateMidnight date,
			IEvent event)
		{

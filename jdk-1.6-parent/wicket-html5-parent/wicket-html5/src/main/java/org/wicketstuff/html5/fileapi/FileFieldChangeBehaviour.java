...
/**
 * <p>
 * A {@link Behavior} that can be used on {@link FileUploadField}-s to receive
 * ajax callback with detailed information about the chosen files when an event
 * occurs (default: "onchange"). When the ajax call is triggered (for example
 * user selects files for upload) the metadata about the file(s) (name, size,
 * type, last modify date) is sent with the ajax call to the server (so the
 * content is not). Subclasses of this class can respond to this event by
 * implementing the {@link #onEvent(AjaxRequestTarget, FileList)} method.
 * </p>
 * <p>
 * Works with single or multiple enabled file inputs.
 * </p>
 * <p>
 * Note that this behaviour only works with browsers implementing the Html5
 * FileApi (Chrome 11, Firefox 4). It does not change or break browsers not
...
public abstract class FileFieldChangeBehaviour extends AjaxEventBehavior {
	...
	public void renderHead(Component component, IHeaderResponse response) {
		...
		response.renderJavaScriptReference(JAVASCRIPT_REF);
	}
	...
	protected void onEvent(AjaxRequestTarget target) {
		...
		for (int fi = 0; fi < num; fi++) {
			...
			String name = parameters.getParameterValue(prefix + "name")
					.toString();
			long size = parameters.getParameterValue(prefix + "size").toLong();
			String type = parameters.getParameterValue(prefix + "type")
					.toString();
			StringValue lastModifiedTimeStr = parameters
	...
	protected CharSequence getCallbackScript() {
		return generateCallbackScript("wicketAjaxPost('"
				+ getCallbackUrl()
	 ...
	 *
	 * @return maximum number of files that are processed from the ajax request,
	 *         non-negative
	 */

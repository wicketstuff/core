...
/**
 * <p>
 * A {@link Behavior} that can be added to {@link FileUploadField}-s to provide
 * a convenient and fast size checking for users using FileApi supporting
 * browsers. Using this has the benefit that the user does not have to upload
 * the whole file data to receive the error about it exceeding the size limit.
 * In browsers not supporting the FileApi this class does not change or break
 * anything (so it is safe for IE users).
 * </p>
 * <p>
 ...
 * Today (2011 first half) browsers do not read the http response until they
 * have fully written out the http request. This means that if you post a form
 * with files in it and wicket stops reading the input because lets say the
 * files are too big, the servlet container will still have to read and throw
 * away the rest of the request before it can send back the response to the
...
public abstract class FileFieldSizeCheckBehaviour extends
	 ...
	 * mimics the upload error handling of Form. In o.a.w.markup.html.form.Form
	 * look for #handleMultiPart() and #onFileUploadException().
	 *
	 * @param target
	...
	protected void addErrorMsg(AjaxRequestTarget target, FileList fileList) {
		...
		String msg = component.getString(component.getId() + "."
				+ UPLOAD_TOO_LARGE_RESOURCE_KEY, Model.ofMap(model),
				defaultValue);
		component.error(msg);
	}

...
goog.exportPath_ = function (a, b, c) {
    ...
    for (var f; a.length && (f = a.shift());)!a.length && void 0 !== b ? c[f] = b : c = c[f] ? c[f] : c[f] = {}
};
...
goog.getObjectByName = function (a, b) {
    for (var c = a.split("."), f = b || goog.global, g; g = c.shift();)if (goog.isDefAndNotNull(f[g]))f = f[g]; else return null;
    return f
};
goog.globalize = function (a, b) {
    var c = b || goog.global, f;
    for (f in a)c[f] = a[f]
};
...
goog.DEPENDENCIES_ENABLED && (goog.included_ = {}, goog.dependencies_ = {pathToNames: {}, nameToPath: {}, requires: {}, visited: {}, written: {}}, goog.inHtmlDocument_ = function () {
    ...
    if (goog.global.CLOSURE_BASE_PATH)goog.basePath = goog.global.CLOSURE_BASE_PATH; else if (goog.inHtmlDocument_())for (var a = goog.global.document.getElementsByTagName("script"), b = a.length - 1; 0 <= b; --b) {
        var c = a[b].src, f = c.lastIndexOf("?"), f = -1 == f ? c.length :
            f;
    ...
    function a(g) {
        if (!(g in f.written)) {
            if (!(g in f.visited) && (f.visited[g] = !0, g in f.requires))for (var k in f.requires[g])if (!goog.isProvided_(k))if (k in f.nameToPath)a(f.nameToPath[k]); else throw Error("Undefined nameToPath for " + k);
            g in c || (c[g] = !0, b.push(g))
        }
    }

    var b = [], c = {}, f = goog.dependencies_, g;
    for (g in goog.included_)f.written[g] || a(g);
    for (g = 0; g < b.length; g++)if (b[g])goog.importScript_(goog.basePath + b[g]); else throw Error("Undefined script input");
...
goog.base = function (a, b, c) {
    ...
    if (goog.DEBUG && !f)throw Error("arguments.caller not defined.  goog.base() expects not to be running in strict mode. See http://www.ecma-international.org/ecma-262/5.1/#sec-C");
    if (f.superClass_)return f.superClass_.constructor.apply(a, Array.prototype.slice.call(arguments, 1));
    for (var g = Array.prototype.slice.call(arguments, 2), h = !1, k = a.constructor; k; k = k.superClass_ && k.superClass_.constructor)if (k.prototype[b] === f)h = !0; else if (h)return k.prototype[b].apply(a,
        g);
...
bay.whiteboard.Collection.prototype.getNeighbourList = function (a, b, c, f) {
    for (var g = [], h = 0; h < this.list.length; h++)if (this.list[h] && this.list[h].distance && (!c || !this.list[h].hidden)) {
        var k = this.list[h].distance(a.x, a.y);
        k <= b && g.push({element: this.list[h], distance: k})
    }
...
bay.whiteboard.Collection.prototype.getJson = function (a) {
    return a.toJson(this.list, this.list.indexOf(a))
};
bay.whiteboard.Collection.prototype.acceptJsonStr = function (a) {
    a = eval("(" + a + ")");
    var b = a.id;
    if (this.list[b])this.list[b].acceptData(a); else {
        var c = bay.whiteboard.Collection.getFromJsonFunc(a.type);
        c && (this.list[b] = c(a, this.list), this.list[b].collection = this)
    }
    return this
};
bay.whiteboard.Collection.prototype.acceptDeletion = function (a) {
    var b = this.list[a];
    return b ? (b.deleteElement(), this.list.length == a + 1 ? this.list.splice(a, 1) : this.list[a] = null, !0) : !1
};
...
bay.whiteboard.Element.prototype.recalcDependant = function () {
    if (this.dependant)for (var a = 0; a < this.dependant.length; a++)this.dependant[a].recalc && this.dependant[a].recalc()
};
...
bay.whiteboard.Point.prototype.draw = function (a) {
    if (this.exists && this.x >= a.area.minX && this.x <= a.area.maxX && this.y >= a.area.minY && this.y <= a.area.maxY) {
        ...
        if (this.current) {
            var c = new goog.graphics.Stroke(a.properties.current.width, a.properties.current.color), f = new goog.graphics.SolidFill(a.properties.current.color);
...
goog.string.subs = function (a, b) {
    for (var c = a.split("%s"), f = "", g = Array.prototype.slice.call(arguments, 1); g.length && 1 < c.length;)f += c.shift() + g.shift();
    return f + c.join("%s")
};
...
goog.string.caseInsensitiveCompare = function (a, b) {
    var c = String(a).toLowerCase(), f = String(b).toLowerCase();
...
goog.string.quote = function (a) {
    ...
    for (var b = ['"'], c = 0; c < a.length; c++) {
        var f = a.charAt(c), g = f.charCodeAt(0);
...
goog.string.compareVersions = function (a, b) {
    for (var c = 0, f = goog.string.trim(String(a)).split("."), g = goog.string.trim(String(b)).split("."), h = Math.max(f.length, g.length), k = 0; 0 == c && k < h; k++) {
        var l = f[k] || "", m = g[k] || "", n = RegExp("(\\d*)(\\D*)", "g"), p = RegExp("(\\d*)(\\D*)", "g");
        do {
            var q = n.exec(l) || ["", "", ""], r = p.exec(m) || ["", "", ""];
            if (0 == q[0].length && 0 == r[0].length)break;
            var c = 0 == q[1].length ? 0 : parseInt(q[1], 10), s = 0 == r[1].length ? 0 : parseInt(r[1], 10), c = goog.string.compareElements_(c, s) || goog.string.compareElements_(0 ==
...
goog.asserts.doAssertFailure_ = function (a, b, c, f) {
    ...
    if (c)var g = g + (": " + c), h = f; else a && (g += ": " + a, h = b);
...
goog.object.some = function (a, b, c) {
    for (var f in a)if (b.call(c, a[f], f, a))return!0;
    return!1
};
goog.object.every = function (a, b, c) {
    for (var f in a)if (!b.call(c, a[f], f, a))return!1;
    return!0
};
...
goog.object.findKey = function (a, b, c) {
    for (var f in a)if (b.call(c, a[f], f, a))return f
};
...
goog.array.filter = goog.NATIVE_ARRAY_PROTOTYPES && goog.array.ARRAY_PROTOTYPE_.filter ? function (a, b, c) {
    ...
    for (var f = a.length, g = [], h = 0, k = goog.isString(a) ? a.split("") : a, l = 0; l < f; l++)if (l in k) {
        var m = k[l];
...
goog.array.some = goog.NATIVE_ARRAY_PROTOTYPES && goog.array.ARRAY_PROTOTYPE_.some ? function (a, b, c) {
    ...
    for (var f = a.length, g = goog.isString(a) ? a.split("") : a, h = 0; h < f; h++)if (h in g && b.call(c, g[h], h, a))return!0;
    return!1
};
goog.array.every = goog.NATIVE_ARRAY_PROTOTYPES && goog.array.ARRAY_PROTOTYPE_.every ? function (a, b, c) {
    ...
    for (var f = a.length, g = goog.isString(a) ? a.split("") : a, h = 0; h < f; h++)if (h in g && !b.call(c, g[h], h, a))return!1;
    return!0
};
...
goog.array.findIndex = function (a, b, c) {
    for (var f = a.length, g = goog.isString(a) ? a.split("") : a, h = 0; h < f; h++)if (h in g && b.call(c, g[h], h, a))return h;
    return-1
};
...
goog.array.findIndexRight = function (a, b, c) {
    for (var f = a.length, g = goog.isString(a) ? a.split("") : a, f = f - 1; 0 <= f; f--)if (f in g && b.call(c, g[f], f, a))return f;
    return-1
};
...
goog.array.remove = function (a, b) {
    var c = goog.array.indexOf(a, b), f;
...
goog.array.extend = function (a, b) {
    for (var c = 1; c < arguments.length; c++) {
        var f = arguments[c], g;
        if (goog.isArray(f) || (g = goog.isArrayLike(f)) && Object.prototype.hasOwnProperty.call(f, "callee"))a.push.apply(a, f); else if (g)for (var h = a.length, k = f.length, l = 0; l < k; l++)a[h + l] = f[l]; else a.push(f)
    }
};
...
goog.array.removeDuplicates = function (a, b) {
    for (var c = b || a, f = {}, g = 0, h = 0; h < a.length;) {
        var k = a[h++], l = goog.isObject(k) ? "o" + goog.getUid(k) : (typeof k).charAt(0) + k;
...
goog.array.equals = function (a, b, c) {
    ...
    for (var g = 0; g < f; g++)if (!c(a[g], b[g]))return!1;
    return!0
};
...
goog.array.range = function (a, b, c) {
    ...
    void 0 !== b && (g = a, h = b);
    if (0 > c * (h - g))return[];
    if (0 < c)for (a = g; a < h; a += c)f.push(a); else for (a = g; a > h; a += c)f.push(a);
    return f
};
...
goog.dom.classes.add = function (a, b) {
    var c = goog.dom.classes.get(a), f = goog.array.slice(arguments, 1), g = c.length + f.length;
    goog.dom.classes.add_(c, f);
    goog.dom.classes.set(a, c.join(" "));
    return c.length == g
};
goog.dom.classes.remove = function (a, b) {
    var c = goog.dom.classes.get(a), f = goog.array.slice(arguments, 1), g = goog.dom.classes.getDifference_(c, f);
...
goog.math.longestCommonSubsequence = function (a, b, c, f) {
    ...
    for (l = 1; l <= g; l++)for (m = 1; m <= h; m++)c(a[l - 1], b[m - 1]) ? k[l][m] = k[l - 1][m - 1] + 1 : k[l][m] = Math.max(k[l - 1][m], k[l][m - 1]);
...
goog.dom.getDocumentHeight_ = function (a) {
    ...
    if (b) {
        ...
        var c = b.body, f = b.documentElement;
        if (goog.dom.isCss1CompatMode_(b) && f.scrollHeight)c = f.scrollHeight != a ? f.scrollHeight : f.offsetHeight; else {
            var b = f.scrollHeight, g = f.offsetHeight;
...
goog.dom.compareNodeOrder = function (a, b) {
    ...
    if ("sourceIndex"in a || a.parentNode && "sourceIndex"in a.parentNode) {
        var c = a.nodeType == goog.dom.NodeType.ELEMENT, f = b.nodeType == goog.dom.NodeType.ELEMENT;
...
goog.dom.findCommonAncestor = function (a) {
    ...
    h = null;
    for (b = 0; b < g; b++) {
        for (var k = f[0][b], l = 1; l < c; l++)if (k != f[l][b])return h;
        h = k
    }
    return h
};
...
goog.dom.getNodeAtOffset = function (a, b, c) {
    a = [a];
    for (var f = 0, g = null; 0 < a.length && f < b;)if (g = a.pop(), !(g.nodeName in goog.dom.TAGS_TO_IGNORE_))if (g.nodeType == goog.dom.NodeType.TEXT)var h = g.nodeValue.replace(/(\r\n|\r|\n)/g, "").replace(/ +/g, " "), f = f + h.length; else if (g.nodeName in goog.dom.PREDEFINED_TAG_VALUES_)f += goog.dom.PREDEFINED_TAG_VALUES_[g.nodeName].length; else for (h = g.childNodes.length - 1; 0 <= h; h--)a.push(g.childNodes[h]);
...
goog.math.Box.distance = function (a, b) {
    var c = goog.math.Box.relativePositionX(a, b), f = goog.math.Box.relativePositionY(a, b);
    return Math.sqrt(c * c + f * f)
};
...
goog.math.Rect.intersection = function (a, b) {
    var c = Math.max(a.left, b.left), f = Math.min(a.left + a.width, b.left + b.width);
    if (c <= f) {
        var g = Math.max(a.top, b.top), h = Math.min(a.top + a.height, b.top + b.height);
...
goog.math.Rect.difference = function (a, b) {
    ...
    var c = [], f = a.top, g = a.height, h = a.left + a.width, k = a.top + a.height, l = b.left + b.width, m = b.top + b.height;
...
goog.style.setPosition = function (a, b, c) {
    var f, g = goog.userAgent.GECKO && (goog.userAgent.MAC || goog.userAgent.X11) && goog.userAgent.isVersionOrHigher("1.9");
...
goog.style.getOffsetParent = function (a) {
    ...
    var b = goog.dom.getOwnerDocument(a), c = goog.style.getStyle_(a, "position"), f = "fixed" == c || "absolute" == c;
    for (a = a.parentNode; a && a != b; a = a.parentNode)if (c = goog.style.getStyle_(a, "position"), f = f && "static" == c && a != b.documentElement && a != b.body, !f && (a.scrollWidth > a.clientWidth || a.scrollHeight > a.clientHeight || "fixed" == c || "absolute" == c || "relative" == c))return a;
    return null
};
goog.style.getVisibleRectForElement = function (a) {
    for (var b = new goog.math.Box(0, Infinity, Infinity, 0), c = goog.dom.getDomHelper(a), f = c.getDocument().body, g = c.getDocument().documentElement, h = c.getDocumentScrollElement(); a = goog.style.getOffsetParent(a);)if ((!goog.userAgent.IE || 0 != a.clientWidth) && (!goog.userAgent.WEBKIT || 0 != a.clientHeight || a != f) && a != f && a != g && "visible" != goog.style.getStyle_(a, "overflow")) {
        var k = goog.style.getPageOffset(a), l = goog.style.getClientLeftTop(a);
        k.x += l.x;
        k.y += l.y;
...
goog.style.getContainerOffsetToScrollInto = function (a, b, c) {
    var f = goog.style.getPageOffset(a), g = goog.style.getPageOffset(b), h = goog.style.getBorderBox(b), k = f.x - g.x - h.left, f = f.y - g.y - h.top, g = b.clientWidth - a.offsetWidth;
...
goog.style.getPageOffset = function (a) {
    var b, c = goog.dom.getOwnerDocument(a), f = goog.style.getStyle_(a, "position");
    goog.asserts.assertObject(a, "Parameter is required");
    var g = !goog.style.GET_BOUNDING_CLIENT_RECT_ALWAYS_EXISTS && goog.userAgent.GECKO && c.getBoxObjectFor && !a.getBoundingClientRect && "absolute" == f && (b = c.getBoxObjectFor(a)) && (0 > b.screenX || 0 > b.screenY), h = new goog.math.Coordinate(0, 0), k = goog.style.getClientViewportElement(c);
    if (a == k)return h;
    if (goog.style.GET_BOUNDING_CLIENT_RECT_ALWAYS_EXISTS ||
        a.getBoundingClientRect)b = goog.style.getBoundingClientRect_(a), a = goog.dom.getDomHelper(c).getDocumentScroll(), h.x = b.left + a.x, h.y = b.top + a.y; else if (c.getBoxObjectFor && !g)b = c.getBoxObjectFor(a), a = c.getBoxObjectFor(k), h.x = b.screenX - a.screenX, h.y = b.screenY - a.screenY; else {
        b = a;
        do {
            h.x += b.offsetLeft;
            h.y += b.offsetTop;
        ...
        for (b = a; (b = goog.style.getOffsetParent(b)) && b != c.body && b != k;)if (h.x -= b.scrollLeft, !goog.userAgent.OPERA || "TR" != b.tagName)h.y -= b.scrollTop
    }
    return h
};
...
goog.style.getFramedPageOffset = function (a, b) {
    var c = new goog.math.Coordinate(0, 0), f = goog.dom.getWindow(goog.dom.getOwnerDocument(a)), g = a;
    do {
        var h = f == b ? goog.style.getPageOffset(g) : goog.style.getClientPositionForElement_(goog.asserts.assert(g));
        c.x += h.x;
        c.y += h.y
...
goog.style.getRelativePosition = function (a, b) {
    var c = goog.style.getClientPosition(a), f = goog.style.getClientPosition(b);
...
goog.style.evaluateWithTemporaryDisplay_ = function (a, b) {
    ...
    var c = b.style, f = c.display, g = c.visibility, h = c.position;
    c.visibility = "hidden";
    c.position = "absolute";
    c.display = "inline";
    var k = a(b);
...
goog.style.getSizeWithDisplay_ = function (a) {
    var b = a.offsetWidth, c = a.offsetHeight, f = goog.userAgent.WEBKIT && !b && !c;
...
goog.style.installStyles = function (a, b) {
    var c = goog.dom.getDomHelper(b), f = null;
    if (goog.userAgent.IE)f = c.getDocument().createStyleSheet(), goog.style.setStyles(f, a); else {
...
goog.style.setUnselectable = function (a, b, c) {
    ...
    } else if (goog.userAgent.IE || goog.userAgent.OPERA)if (b = b ? "on" : "", a.setAttribute("unselectable", b), c)for (a = 0; g = c[a]; a++)g.setAttribute("unselectable", b)
};
goog.style.getBorderBoxSize = function (a) {
    return new goog.math.Size(a.offsetWidth, a.offsetHeight)
};
goog.style.setBorderBoxSize = function (a, b) {
    var c = goog.dom.getOwnerDocument(a), f = goog.dom.getDomHelper(c).isCss1CompatMode();
    if (goog.userAgent.IE && (!f || !goog.userAgent.isVersionOrHigher("8")))if (c = a.style, f) {
        var f = goog.style.getPaddingBox(a), g = goog.style.getBorderBox(a);
...
goog.style.setContentBoxSize = function (a, b) {
    var c = goog.dom.getOwnerDocument(a), f = goog.dom.getDomHelper(c).isCss1CompatMode();
    if (goog.userAgent.IE && (!f || !goog.userAgent.isVersionOrHigher("8")))if (c = a.style, f)c.pixelWidth = b.width, c.pixelHeight = b.height; else {
        var f = goog.style.getPaddingBox(a), g = goog.style.getBorderBox(a);
        c.pixelWidth = b.width + g.left + f.left + f.right + g.right;
        c.pixelHeight = b.height + g.top + f.top + f.bottom + g.bottom
...
goog.style.getIePixelValue_ = function (a, b, c, f) {
    ...
    var g = a.style[c], h = a.runtimeStyle[c];
...
goog.style.getBox_ = function (a, b) {
    if (goog.userAgent.IE) {
        var c = goog.style.getIePixelDistance_(a, b + "Left"), f = goog.style.getIePixelDistance_(a, b + "Right"), g = goog.style.getIePixelDistance_(a, b + "Top"), h = goog.style.getIePixelDistance_(a, b + "Bottom");
...
goog.style.getBorderBox = function (a) {
    if (goog.userAgent.IE) {
        var b = goog.style.getIePixelBorder_(a, "borderLeft"), c = goog.style.getIePixelBorder_(a, "borderRight"), f = goog.style.getIePixelBorder_(a, "borderTop");
...
goog.events.Listener = function (a, b, c, f, g, h) {
...
goog.events.listen = function (a, b, c, f, g) {
    ...
    return goog.events.Listenable.isImplementedBy(a) ? a.listen(b, c, f, g) : goog.events.listen_(a, b, c, !1, f, g)
};
goog.events.listen_ = function (a, b, c, f, g, h) {
    if (!b)throw Error("Invalid event type");
    g = !!g;
    var k = goog.events.listenerTree_;
    b in k || (k[b] = {count_: 0, remaining_: 0});
    k = k[b];
    g in k || (k[g] = {count_: 0, remaining_: 0}, k.count_++);
    var k = k[g], l = goog.getUid(a), m;
    k.remaining_++;
    if (k[l]) {
        m = k[l];
        for (var n = 0; n < m.length; n++)if (k = m[n], k.listener == c && k.handler == h) {
            if (k.removed)break;
            f || (m[n].callOnce = !1);
            return m[n]
        }
    ...
    k = new goog.events.Listener(c, n, a, b, g, h);
    k.callOnce = f;
    n.src = a;
    n.listener = k;
    m.push(k);
    ...
    a.addEventListener ? a.addEventListener(b, n, g) : a.attachEvent(goog.events.getOnString_(b), n);
    return goog.events.listeners_[k.key] = k
};
...
goog.events.listenOnce = function (a, b, c, f, g) {
    ...
    c = goog.events.wrapListener_(c);
    return goog.events.Listenable.isImplementedBy(a) ? a.listenOnce(b, c, f, g) : goog.events.listen_(a, b, c, !0, f, g)
};
...
goog.events.unlisten = function (a, b, c, f, g) {
    ...
    c = goog.events.wrapListener_(c);
    if (goog.events.Listenable.isImplementedBy(a))return a.unlisten(b, c, f, g);
    f = !!f;
    a = goog.events.getListeners_(a, b, f);
    if (!a)return!1;
    for (h = 0; h < a.length; h++)if (a[h].listener == c && a[h].capture == f && a[h].handler == g)return goog.events.unlistenByKey(a[h]);
    return!1
};
goog.events.unlistenByKey = function (a) {
    ...
    var c = a.type, f = a.proxy, g = a.capture;
    b.removeEventListener ? b.removeEventListener(c, f, g) : b.detachEvent && b.detachEvent(goog.events.getOnString_(c), f);
    b = goog.getUid(b);
    goog.events.sources_[b] && (f = goog.events.sources_[b], goog.array.remove(f, a), 0 == f.length && delete goog.events.sources_[b]);
    a.markAsRemoved();
    if (f = goog.events.listenerTree_[c][g][b])goog.array.remove(f,
        a), 0 == f.length && (delete goog.events.listenerTree_[c][g][b], goog.events.listenerTree_[c][g].count_--), 0 == goog.events.listenerTree_[c][g].count_ && (delete goog.events.listenerTree_[c][g], goog.events.listenerTree_[c].count_--), 0 == goog.events.listenerTree_[c].count_ && delete goog.events.listenerTree_[c];
...
goog.events.removeAll = function (a, b) {
    ...
    if (null != a) {
        ...
        var g = goog.getUid(a);
        if (goog.events.sources_[g])for (var g = goog.events.sources_[g], h = g.length - 1; 0 <= h; h--) {
            var k = g[h];
            if (f || b == k.type)goog.events.unlistenByKey(k), c++
        }
...
goog.events.getListener = function (a, b, c, f, g) {
    f = !!f;
    c = goog.events.wrapListener_(c);
    if (goog.events.Listenable.isImplementedBy(a))return a.getListener(b, c, f, g);
    if (a = goog.events.getListeners_(a, b, f))for (b = 0; b < a.length; b++)if (!a[b].removed && a[b].listener == c && a[b].capture == f && a[b].handler == g)return a[b];
    return null
};
goog.events.hasListener = function (a, b, c) {
    ...
    if (f) {
        var g = goog.isDef(b), h = goog.isDef(c);
...
goog.events.fireListeners = function (a, b, c, f) {
    if (goog.events.Listenable.isImplementedBy(a))return a.fireListeners(b, c, f);
...
goog.events.fireListeners_ = function (a, b, c, f, g) {
    ...
    if (a[f]) {
        b = --a.remaining_;
        f = goog.array.clone(a[f]);
        try {
            for (var h = 0; h < f.length; h++) {
                var k = f[h];
                k && !k.removed && (c &= !1 !== goog.events.fireListener(k, g))
            }
        } finally {
            a.remaining_ = Math.max(b, a.remaining_)
        }
    }
    return Boolean(c)
};
goog.events.fireListener = function (a, b) {
    var c = a.listener, f = a.handler || a.src;
...
goog.events.handleBrowserEvent_ = function (a, b) {
    ...
    var c = a.type, f = goog.events.listenerTree_;
    if (!(c in f))return!0;
    var f = f[c], g, h;
...
goog.events.EventHandler.prototype.listen = function (a, b, c, f, g) {
    ...
    for (var h = 0; h < b.length; h++) {
        ...
        if (goog.DEBUG && !k)break;
        this.keys_[k.key] = k
    }
    return this
};
goog.events.EventHandler.prototype.listenOnce = function (a, b, c, f, g) {
    if (goog.isArray(b))for (var h = 0; h < b.length; h++)this.listenOnce(a, b[h], c, f, g); else a = goog.events.listenOnce(a, b, c || this, f, g || this.handler_ || this), this.keys_[a.key] = a;
    return this
};
...
goog.events.EventHandler.prototype.unlisten = function (a, b, c, f, g) {
    if (goog.isArray(b))for (var h = 0; h < b.length; h++)this.unlisten(a, b[h], c, f, g); else if (a = goog.events.getListener(a, b, c || this, f, g || this.handler_ || this))goog.events.unlistenByKey(a), delete this.keys_[a.key];
    return this
};
...
goog.style.bidi.getScrollLeft = function (a) {
    ...
    return b && goog.userAgent.GECKO ? -a.scrollLeft : b && (!goog.userAgent.IE || !goog.userAgent.isVersionOrHigher("8")) && "visible" != goog.style.getComputedOverflowX(a) ? a.scrollWidth - a.clientWidth - a.scrollLeft : a.scrollLeft
};
goog.style.bidi.getOffsetStart = function (a) {
    ...
    if (!c)return b;
    if (goog.userAgent.GECKO)var f = goog.style.getBorderBox(c), b = b + f.left; else goog.userAgent.isDocumentModeOrHigher(8) && (f = goog.style.getBorderBox(c), b -= f.left);
    return goog.style.isRightToLeft(c) ? c.clientWidth - (b + a.offsetWidth) : b
};
goog.style.bidi.setScrollOffset = function (a, b) {
    b = Math.max(b, 0);
    goog.style.isRightToLeft(a) ? goog.userAgent.GECKO ? a.scrollLeft = -b : !goog.userAgent.IE || !goog.userAgent.isVersionOrHigher("8") ? a.scrollLeft = a.scrollWidth - b - a.clientWidth : a.scrollLeft = b : a.scrollLeft = b
};
...
goog.events.EventTarget.prototype.listen = function (a, b, c, f) {
...
goog.events.EventTarget.prototype.listenInternal_ = function (a, b, c, f, g) {
    this.assertInitialized_();
    var h = this.eventTargetListeners_[a] || (this.eventTargetListeners_[a] = []), k = goog.events.EventTarget.findListenerIndex_(h, b, f, g);
    if (-1 < k)return a = h[k], c || (a.callOnce = !1), a;
    a = new goog.events.Listener(b, null, this, a, !!f, g);
    a.callOnce = c;
    h.push(a);
    return a
};
...
goog.events.EventTarget.prototype.unlistenByKey = function (a) {
    ...
    if (b = goog.array.remove(this.eventTargetListeners_[b], a))a.removed = !0;
    return b
};
goog.events.EventTarget.prototype.removeAllListeners = function (a, b) {
    var c = 0, f;
    for (f in this.eventTargetListeners_)if (!a || f == a) {
        for (var g = this.eventTargetListeners_[f], h = 0; h < g.length; h++)++c, g[h].removed = !0;
        g.length = 0
    }
    return c
};
...
goog.events.EventTarget.prototype.getListeners = function (a, b) {
    var c = this.eventTargetListeners_[a], f = [];
...
goog.events.EventTarget.prototype.getListener = function (a, b, c, f) {
    a = this.eventTargetListeners_[a];
    var g = -1;
...
goog.events.EventTarget.prototype.hasListener = function (a, b) {
    var c = goog.isDef(a), f = goog.isDef(b);
    return goog.object.some(this.eventTargetListeners_, function (g, h) {
        for (var k = 0; k < g.length; ++k)if ((!c || g[k].type == a) && (!f || g[k].capture == b))return!0;
        return!1
    })
};
...
goog.events.EventTarget.dispatchEventInternal_ = function (a, b, c) {
    ...
    var g = !0, h;
    if (c)for (var k = c.length - 1; !b.propagationStopped_ && 0 <= k; k--)h = b.currentTarget = c[k], g = h.fireListeners(f, !0, b) && g;
    b.propagationStopped_ || (h = b.currentTarget = a, g = h.fireListeners(f, !0, b) && g, b.propagationStopped_ || (g = h.fireListeners(f, !1, b) && g));
    if (c)for (k = 0; !b.propagationStopped_ && k < c.length; k++)h = b.currentTarget = c[k], g = h.fireListeners(f, !1, b) && g;
    return g
};
...
goog.fx.Dragger.prototype.endDrag = function (a, b) {
    ...
    if (this.dragging_) {
        this.maybeReinitTouchEvent_(a);
        this.dragging_ = !1;
        var c = this.limitX(this.deltaX), f = this.limitY(this.deltaY);
...
goog.fx.Dragger.prototype.handleMove_ = function (a) {
    if (this.enabled_) {
        ...
        this.screenY = a.screenY;
        if (!this.dragging_) {
            var f = this.startX - this.clientX, g = this.startY - this.clientY;
            if (f * f + g * g > this.hysteresisDistanceSquared_)if (this.fireDragStart_(a))this.dragging_ = !0; else {
...
goog.ui.Component.prototype.decorate = function (a) {
    ...
    if (a && this.canDecorate(a)) {
        ...
        if (!this.dom_ || this.dom_.getDocument() != b)this.dom_ = goog.dom.getDomHelper(a);
...
goog.a11y.aria.setState = function (a, b, c) {
    ...
    var f = goog.a11y.aria.getAriaAttributeName_(b);
    "" === c || void 0 == c ? (c = goog.a11y.aria.datatables.getDefaultValuesMap(), b in c ? a.setAttribute(f, c[b]) : a.removeAttribute(f)) : a.setAttribute(f, c)
};
...
goog.dom.iframe.createWithContent = function (a, b, c, f, g) {
    var h = goog.dom.getDomHelper(a), k = [];
...
goog.ui.ModalPopup.prototype.reposition = function () {
    ...
    var f = goog.style.getSize(this.getElement()), b = goog.dom.getViewportSize(b), a = Math.max(a + b.width / 2 - f.width / 2, 0), c = Math.max(c + b.height / 2 - f.height / 2, 0);
...
goog.iter.chain = function (a) {
    var b = arguments, c = b.length, f = 0, g = new goog.iter.Iterator;
    g.next = function () {
        try {
...
goog.iter.product = function (a) {
    ...
    var b = new goog.iter.Iterator, c = arguments, f = goog.array.repeat(0, c.length);
    b.next = function () {
        if (f) {
...
goog.iter.cycle = function (a) {
    var b = goog.iter.toIterator(a), c = [], f = 0;
    a = new goog.iter.Iterator;
    var g = !1;
    a.next = function () {
        var a = null;
...
goog.structs.Map.prototype.equals = function (a, b) {
    ...
    this.cleanupKeysArray_();
    for (var f, g = 0; f = this.keys_[g]; g++)if (!c(this.get(f), a.get(f)))return!1;
    return!0
};
...
goog.structs.Map.prototype.__iterator__ = function (a) {
    this.cleanupKeysArray_();
    var b = 0, c = this.keys_, f = this.map_, g = this.version_, h = this, k = new goog.iter.Iterator;
    k.next = function () {
        for (; ;) {
...
goog.structs.forEach = function (a, b, c) {
    if ("function" == typeof a.forEach)a.forEach(b, c); else if (goog.isArrayLike(a) || goog.isString(a))goog.array.forEach(a, b, c); else for (var f = goog.structs.getKeys(a), g = goog.structs.getValues(a), h = g.length, k = 0; k < h; k++)b.call(c, g[k], f && f[k], a)
};
goog.structs.filter = function (a, b, c) {
    ...
    var f, g = goog.structs.getKeys(a), h = goog.structs.getValues(a), k = h.length;
    if (g) {
        f = {};
...
goog.structs.some = function (a, b, c) {
    ...
    for (var f = goog.structs.getKeys(a), g = goog.structs.getValues(a), h = g.length, k = 0; k < h; k++)if (b.call(c, g[k], f && f[k], a))return!0;
    return!1
};
goog.structs.every = function (a, b, c) {
    if ("function" == typeof a.every)return a.every(b, c);
    if (goog.isArrayLike(a) || goog.isString(a))return goog.array.every(a, b, c);
    for (var f = goog.structs.getKeys(a), g = goog.structs.getValues(a), h = g.length, k = 0; k < h; k++)if (!b.call(c, g[k], f && f[k], a))return!1;
    return!0
};
...
goog.ui.Dialog.prototype.setModalInternal_ = function (a) {
    ...
    if (this.isInDocument()) {
        var b = this.getDomHelper(), c = this.getBackgroundElement(), f = this.getBackgroundIframe();
...
goog.ui.Dialog.prototype.decorateInternal = function (a) {
    ...
    var b = this.class_ + "-title",
        c = this.class_ + "-title-text", f = this.class_ + "-title-close";
...
goog.ui.Dialog.prototype.focus = function () {
    ...
    if (this.getButtonSet()) {
        ...
        if (a)for (var b = this.getDomHelper().getDocument(), c = this.buttonEl_.getElementsByTagName("button"), f = 0, g; g = c[f]; f++)if (g.name == a && !g.disabled) {
            try {
...
goog.ui.Dialog.prototype.setDraggerLimits_ = function (a) {
    ...
    var c = Math.max(b.body.scrollWidth, a.width), b = Math.max(b.body.scrollHeight, a.height), f = goog.style.getSize(this.getElement());
...
goog.ui.Dialog.prototype.onKey_ = function (a) {
    var b = !1, c = !1, f = this.getButtonSet(), g = a.target;
    if (a.type == goog.events.EventType.KEYDOWN)if (this.escapeToCancel_ && a.keyCode == goog.events.KeyCodes.ESC) {
        var h = f && f.getCancel(), g = "SELECT" == g.tagName && !g.disabled;
        h && !g ? (c = !0, b = f.get(h), b = this.dispatchEvent(new goog.ui.Dialog.Event(h, b))) : g || (b = !0)
    } else a.keyCode == goog.events.KeyCodes.TAB && (a.shiftKey && g == this.getElement()) && this.setupBackwardTabWrap(); else if (a.keyCode == goog.events.KeyCodes.ENTER) {
        if ("BUTTON" ==
            g.tagName && !g.disabled)h = g.name; else if (f) {
            var k = f.getDefault(), l = k && f.getButton(k), g = ("TEXTAREA" == g.tagName || "SELECT" == g.tagName || "A" == g.tagName) && !g.disabled;
            l && (!l.disabled && !g) && (h = k)
        }
...
goog.ui.Dialog.ButtonSet.prototype.render = function () {
    if (this.element_) {
        ...
        goog.structs.forEach(this, function (b, c) {
            var f = a.createDom("button", {name: c}, b);
...
goog.ui.Dialog.ButtonSet.prototype.decorate = function (a) {
    if (a && a.nodeType == goog.dom.NodeType.ELEMENT) {
        ...
        for (var b = 0, c, f, g; c = a[b]; b++)if (f = c.name || c.id, g = goog.dom.getTextContent(c) || c.value, f) {
            var h = 0 == b;
...
goog.ui.Dialog.ButtonSet.prototype.getButton = function (a) {
    for (var b = this.getAllButtons(), c = 0, f; f = b[c]; c++)if (f.name == a || f.id == a)return f;
    return null
};
...
goog.ui.BidiInput.prototype.setDirection_ = function () {
    var a = this.getElement(), b = this.getValue();
...
goog.ui.BidiInput.prototype.setValue = function (a) {
    ...
    goog.isDefAndNotNull(b.value) ? b.value = a : goog.dom.setTextContent(b, a);
    this.setDirection_()
};
goog.ui.BidiInput.prototype.getValue = function () {
    var a = this.getElement();
    return goog.isDefAndNotNull(a.value) ? a.value : goog.dom.getRawTextContent(a)
};
...
goog.ui.ControlRenderer.prototype.decorate = function (a, b) {
    ...
    var f = 0, g = this.getCssClass(), h = this.getStructuralCssClass(), k = !1, l = !1, c = !1, m = goog.dom.classes.get(b);
...
goog.ui.ControlRenderer.prototype.getClassNames = function (a) {
    var b = this.getCssClass(), c = [b], f = this.getStructuralCssClass();
    f != b && c.push(f);
...
goog.ui.CheckboxRenderer.prototype.decorate = function (a, b) {
    ...
    var c = goog.dom.classes.get(b), f = goog.ui.Checkbox.State.UNCHECKED;
...
goog.ui.registry.getDecorator = function (a) {
    for (var b = goog.dom.classes.get(a), c = 0, f = b.length; c < f; c++)if (a = goog.ui.registry.getDecoratorByClassName(b[c]))return a;
    return null
};
...
goog.events.KeyHandler.prototype.handleKeyDown_ = function (a) {
    if (goog.userAgent.WEBKIT && (this.lastKey_ == goog.events.KeyCodes.CTRL && !a.ctrlKey || this.lastKey_ == goog.events.KeyCodes.ALT && !a.altKey || goog.userAgent.MAC && this.lastKey_ == goog.events.KeyCodes.META && !a.metaKey))this.keyCode_ = this.lastKey_ = -1;
...
goog.events.KeyHandler.prototype.handleEvent = function (a) {
    var b = a.getBrowserEvent(), c, f, g = b.altKey;
...
goog.ui.ButtonRenderer.prototype.setCollapsed = function (a, b) {
    var c = a.isRightToLeft(), f = this.getStructuralCssClass() + "-collapse-left", g = this.getStructuralCssClass() + "-collapse-right";
...
goog.debug.expose = function (a, b) {
    ...
    var c = [], f;
    for (f in a)if (b || !goog.isFunction(a[f])) {
        var g = f + " = ";
        try {
            g += a[f]
        } catch (h) {
...
goog.debug.deepExpose = function (a, b) {
    var c = new goog.structs.Set, f = [], g = function (a, k) {
        var l = k + "  ";
        try {
            if (goog.isDef(a))if (goog.isNull(a))f.push("NULL"); else if (goog.isString(a))f.push('"' + a.replace(/\n/g, "\n" + k) + '"'); else if (goog.isFunction(a))f.push(String(a).replace(/\n/g, "\n" + k)); else if (goog.isObject(a))if (c.contains(a))f.push("*** reference loop detected ***"); else {
                c.add(a);
                f.push("{");
                for (var m in a)if (b || !goog.isFunction(a[m]))f.push("\n"), f.push(l), f.push(m + " = "), g(a[m], l);
                f.push("\n" + k + "}")
            } else f.push(a);
...
goog.debug.normalizeErrorObject = function (a) {
    ...
    return g || !a.lineNumber || !a.fileName || !a.stack ? {message: a.message, name: a.name, lineNumber: c, fileName: f,
        stack: a.stack || "Not available"} : a
};
...
goog.debug.LogBuffer.prototype.forEachRecord = function (a) {
    var b = this.buffer_;
    if (b[0]) {
        var c = this.curIndex_, f = this.isFull_ ? c : -1;
...
goog.debug.LogManager.createLogger_ = function (a) {
    ...
    if (goog.debug.Logger.ENABLE_HIERARCHY) {
        var c = a.lastIndexOf("."), f = a.substr(0, c), c = a.substr(c + 1), f = goog.debug.LogManager.getLogger(f);
        f.addChild_(c, b);
        b.setParent_(f)
    }
...
goog.log.log = function (a, b, c, f) {
    goog.log.ENABLED && a && a.log(b, c, f)
};
...
goog.net.WebSocket.prototype.onClose_ = function (a) {
    ...
    this.closeExpected_ ? (goog.log.info(this.logger_, "The WebSocket closed normally."), this.url_ = null, this.protocol_ = void 0) : (goog.log.error(this.logger_, "The WebSocket disconnected unexpectedly: " + a.data), this.autoReconnect_ && (goog.log.info(this.logger_, "Seconds until next reconnect attempt: " + Math.floor(this.nextReconnect_ /
        1E3)), this.reconnectTimer_ = goog.Timer.callOnce(goog.bind(this.open, this, this.url_, this.protocol_), this.nextReconnect_, this), this.reconnectAttempt_++, this.nextReconnect_ = this.getNextReconnect_(this.reconnectAttempt_)));
...
goog.positioning.positionAtAnchor = function (a, b, c, f, g, h, k, l, m) {
    goog.asserts.assert(c);
    var n = goog.positioning.getOffsetParentPageOffset(c), p = goog.positioning.getVisiblePart_(a);
    ...
    var q;
    if (k)if (m)q = m; else if (q = goog.style.getVisibleRectForElement(c))q.top -= n.y, q.right -= n.x, q.bottom -= n.y, q.left -= n.x;
...
goog.positioning.adjustForViewport_ = function (a, b, c, f) {
    var g = goog.positioning.OverflowStatus.NONE, h = goog.positioning.Overflow.ADJUST_X_EXCEPT_OFFSCREEN, k = goog.positioning.Overflow.ADJUST_Y_EXCEPT_OFFSCREEN;
    ...
    a.y <= c.top && (a.y + b.height < c.bottom && f & goog.positioning.Overflow.RESIZE_HEIGHT) && (b.height = Math.max(b.height - (c.top - a.y), 0), a.y = c.top, g |= goog.positioning.OverflowStatus.HEIGHT_ADJUSTED);
...
goog.positioning.ClientPosition.prototype.reposition = function (a, b, c, f) {
    goog.asserts.assert(a);
    var g = goog.style.getViewportPageOffset(goog.dom.getOwnerDocument(a)), h = this.coordinate.x + g.x, g = this.coordinate.y + g.y, k = goog.positioning.getOffsetParentPageOffset(a), h = h - k.x, g = g - k.y;
...
goog.positioning.ViewportClientPosition.prototype.reposition = function (a, b, c, f) {
    var g = goog.style.getClientViewportElement(a), g = goog.style.getVisibleRectForElement(g), h = goog.dom.getDomHelper(a).getDocumentScrollElement(), h = new goog.math.Coordinate(this.coordinate.x + h.scrollLeft, this.coordinate.y + h.scrollTop), k = goog.positioning.Overflow.FAIL_X | goog.positioning.Overflow.FAIL_Y, l = b, m = goog.positioning.positionAtCoordinate(h, a, l, c, g, k, f);
...
goog.ui.KeyboardShortcutHandler.prototype.interpretStrokes_ = function (a, b) {
    var c;
    if (goog.isString(b[a]))c = goog.ui.KeyboardShortcutHandler.parseStringShortcut(b[a]); else {
        var f = b, g = a;
        goog.isArray(b[a]) && (f = b[a], g = 0);
        for (c = []; g < f.length; g += 2)c.push({keyCode: f[g], modifiers: f[g + 1]})
    }
    return c
};
...
goog.ui.KeyboardShortcutHandler.prototype.initializeKeyListener = function (a) {
    ...
    goog.userAgent.MAC && (goog.userAgent.GECKO && goog.userAgent.isVersionOrHigher("1.8")) && goog.events.listen(this.keyTarget_, goog.events.EventType.KEYUP, this.handleMacGeckoKeyUp_, !1, this);
    goog.userAgent.WINDOWS && !goog.userAgent.GECKO && (goog.events.listen(this.keyTarget_, goog.events.EventType.KEYPRESS, this.handleWindowsKeyPress_,
...
goog.ui.KeyboardShortcutHandler.prototype.clearKeyListener = function () {
    ...
    goog.userAgent.MAC && (goog.userAgent.GECKO && goog.userAgent.isVersionOrHigher("1.8")) && goog.events.unlisten(this.keyTarget_, goog.events.EventType.KEYUP, this.handleMacGeckoKeyUp_, !1, this);
    goog.userAgent.WINDOWS && !goog.userAgent.GECKO && (goog.events.unlisten(this.keyTarget_, goog.events.EventType.KEYPRESS, this.handleWindowsKeyPress_, !1, this),
        goog.events.unlisten(this.keyTarget_, goog.events.EventType.KEYUP, this.handleWindowsKeyUp_, !1, this));
    this.keyTarget_ = null
};
goog.ui.KeyboardShortcutHandler.setShortcut_ = function (a, b, c) {
    var f = b.shift(), f = goog.ui.KeyboardShortcutHandler.makeKey_(f.keyCode, f.modifiers), g = a[f];
...
goog.ui.KeyboardShortcutHandler.prototype.checkShortcut_ = function (a) {
    for (var b = this.shortcuts_; 0 < a.length && b;) {
        var c = a.shift(), c = goog.ui.KeyboardShortcutHandler.makeKey_(c.keyCode, c.modifiers), b = b[c];
...
goog.ui.KeyboardShortcutHandler.prototype.handleKeyDown_ = function (a) {
    if (this.isValidShortcut_(a))if ("keydown" == a.type && this.isPossiblePrintableKey_(a))this.isPrintableKey_ = !1; else {
        var b = goog.userAgent.GECKO ? goog.events.KeyCodes.normalizeGeckoKeyCode(a.keyCode) : a.keyCode, b = goog.ui.KeyboardShortcutHandler.makeKey_(b, (a.shiftKey ? goog.ui.KeyboardShortcutHandler.Modifiers.SHIFT : 0) | (a.ctrlKey ? goog.ui.KeyboardShortcutHandler.Modifiers.CTRL : 0) | (a.altKey ? goog.ui.KeyboardShortcutHandler.Modifiers.ALT : 0) | (a.metaKey ?
            goog.ui.KeyboardShortcutHandler.Modifiers.META : 0)), c, f, g = goog.now();
...
goog.ui.KeyboardShortcutHandler.prototype.isValidShortcut_ = function (a) {
    ...
    var c = a.target, f = "TEXTAREA" == c.tagName || "INPUT" == c.tagName || "BUTTON" == c.tagName || "SELECT" == c.tagName, g = !f && (c.isContentEditable || c.ownerDocument && "on" == c.ownerDocument.designMode);
    return!f && !g || this.globalKeys_[b] || this.allShortcutsAreGlobal_ ? !0 : g ? !1 : this.modifierShortcutsAreGlobal_ && (a.altKey || a.ctrlKey || a.metaKey) ?
        !0 : "INPUT" == c.tagName && this.textInputs_[c.type] ? b == goog.events.KeyCodes.ENTER : "INPUT" == c.tagName || "BUTTON" == c.tagName ? b != goog.events.KeyCodes.SPACE : !1
};
...
goog.ui.ContainerRenderer.prototype.decorate = function (a, b) {
    b.id && a.setId(b.id);
    var c = this.getCssClass(), f = !1, g = goog.dom.classes.get(b);
...
goog.ui.MenuItemRenderer.prototype.setContent = function (a, b) {
    var c = this.getContentElement(a), f = this.hasCheckBoxStructure(a) ? c.firstChild : null;
...
goog.ui.CustomButtonRenderer.prototype.hasBoxStructure = function (a, b) {
    var c = a.getDomHelper().getFirstElementChild(b), f = this.getCssClass() + "-outer-box";
...
goog.ui.CustomButtonRenderer.trimTextNodes_ = function (a, b) {
    if (a)for (var c = b ? a.firstChild : a.lastChild, f; c && c.parentNode == a;) {
        f = b ? c.nextSibling : c.previousSibling;
        if (c.nodeType == goog.dom.NodeType.TEXT) {
            var g = c.nodeValue;
            if ("" == goog.string.trim(g))a.removeChild(c); else {
...
goog.ui.MenuButton.prototype.attachMenuEventListeners_ = function (a, b) {
    var c = this.getHandler(), f = b ? c.listen : c.unlisten;
...
goog.color.rgbToHsl = function (a, b, c) {
    ...
    b /= 255;
    c /= 255;
    var f = Math.max(a, b, c), g = Math.min(a, b, c), h = 0, k = 0, l = 0.5 * (f + g);
...
goog.color.hslToRgb = function (a, b, c) {
    var f = 0, g = 0, h = 0;
    a /= 360;
    if (0 == b)f = g = h = 255 * c; else var k = h = 0, k = 0.5 > c ? c * (1 + b) : c + b - b * c, h = 2 * c - k, f = 255 * goog.color.hueToRgb_(h, k, a + 1 / 3), g = 255 * goog.color.hueToRgb_(h, k, a), h = 255 * goog.color.hueToRgb_(h, k, a - 1 / 3);
...
goog.color.rgbToHsv = function (a, b, c) {
    var f = Math.max(Math.max(a, b), c), g = Math.min(Math.min(a, b), c);
    if (g == f)g = a = 0; else {
        var h = f - g, g = h / f;
...
goog.color.highContrast = function (a, b) {
    for (var c = [], f = 0; f < b.length; f++)c.push({color: b[f], diff: goog.color.yiqBrightnessDiff_(b[f], a) + goog.color.colorDiff_(b[f], a)});
    c.sort(function (a, b) {
...
goog.ui.PaletteRenderer.prototype.createCell = function (a, b) {
    ...
    if (!goog.dom.getTextContent(c) && !goog.a11y.aria.getLabel(c)) {
        var f = this.findAriaLabelForCell_(c);
        f && goog.a11y.aria.setLabel(c, f)
    }
    return c
};
...
goog.ui.PaletteRenderer.prototype.highlightCell = function (a, b, c) {
    b && (b = this.getCellForItem(b), goog.dom.classes.enable(b, this.getCssClass() + "-cell-hover", c), a = a.getElement().firstChild, goog.a11y.aria.setState(a, goog.a11y.aria.State.ACTIVEDESCENDANT, b.id))
};
...
goog.events.MouseWheelHandler.prototype.handleEvent = function (a) {
    ...
    } else f = a.detail, 100 < f ? f = 3 : -100 > f && (f = -3), goog.isDef(a.axis) && a.axis === a.HORIZONTAL_AXIS ? b = f : c = f;
...
goog.ui.CustomColorPalette.prototype.createColorNodes = function () {
    var a = goog.getMsg("Add a color"), b = goog.ui.CustomColorPalette.superClass_.createColorNodes.call(this);
    b.push(goog.dom.createDom("div", {"class": "goog-palette-customcolor", title: a}, "+"));
    return b
};
goog.ui.CustomColorPalette.prototype.performActionInternal = function (a) {
    if (a = this.getHighlightedItem())if (goog.dom.classes.has(a, "goog-palette-customcolor"))this.promptForCustomColor(); else return this.setSelectedItem(a), this.dispatchEvent(goog.ui.Component.EventType.ACTION);
    return!1
};
goog.ui.CustomColorPalette.prototype.promptForCustomColor = function () {
    var a = goog.getMsg("Input custom color, i.e. pink, #F00, #D015FF or rgb(100, 50, 25)"), b = null;
    this.setOpen(!0);
    ...
    if (b) {
        ...
        try {
            ...
            c = goog.getMsg('ERROR: "{$color}" is not a valid color.', {color: b});
            alert(c);
            return
        }
        a = this.getColors();
...
goog.graphics.AffineTransform = function (a, b, c, f, g, h) {
    if (6 == arguments.length)this.setTransform(a, b, c, f, g, h); else {
...
goog.graphics.AffineTransform.prototype.preShear = function (a, b) {
    var c = this.m00_, f = this.m01_, g = this.m02_;
...
goog.graphics.SvgGroupElement.prototype.setSize = function (a, b) {
    this.getGraphics().setElementAttributes(this.getElement(), {width: a, height: b})
};
...
goog.graphics.SvgEllipseElement.prototype.setCenter = function (a, b) {
    this.getGraphics().setElementAttributes(this.getElement(), {cx: a, cy: b})
};
goog.graphics.SvgEllipseElement.prototype.setRadius = function (a, b) {
    this.getGraphics().setElementAttributes(this.getElement(), {rx: a, ry: b})
};
...
goog.graphics.SvgRectElement.prototype.setPosition = function (a, b) {
    this.getGraphics().setElementAttributes(this.getElement(), {x: a, y: b})
};
...
goog.graphics.SvgPathElement.prototype.setPath = function (a) {
    this.getGraphics().setElementAttributes(this.getElement(), {d: goog.graphics.SvgGraphics.getSvgPath(a)})
};
...
goog.graphics.SvgImageElement.prototype.setSource = function (a) {
    this.getGraphics().setElementAttributes(this.getElement(), {"xlink:href": a})
};
...
goog.graphics.Path.prototype.lineTo = function (a) {
    ...
    for (b = 0; b < arguments.length; b += 2) {
        var c = arguments[b], f = arguments[b + 1];
...
goog.graphics.Path.prototype.curveTo = function (a) {
    ...
    for (b = 0; b < arguments.length; b += 6) {
        var c = arguments[b + 4], f = arguments[b + 5];
...
goog.graphics.Path.prototype.arcTo = function (a, b, c, f) {
    var g = this.currentPoint_[0] - goog.math.angleDx(c, a), h = this.currentPoint_[1] - goog.math.angleDy(c, b), g = g + goog.math.angleDx(c + f, a), h = h + goog.math.angleDy(c + f, b);
...
goog.graphics.Path.prototype.arcToAsCurves = function (a, b, c, f) {
    var g = this.currentPoint_[0] - goog.math.angleDx(c, a), h = this.currentPoint_[1] - goog.math.angleDy(c, b), k = goog.math.toRadians(f);
    f = Math.ceil(2 * (Math.abs(k) / Math.PI));
    k /= f;
    c = goog.math.toRadians(c);
    for (var l = 0; l < f; l++) {
        var m = Math.cos(c), n = Math.sin(c), p = 4 / 3 * Math.sin(k / 2) / (1 + Math.cos(k / 2)), q = g + (m - p * n) * a, r = h + (n + p * m) * b;
        c += k;
        m = Math.cos(c);
        n = Math.sin(c);
...
goog.graphics.Path.prototype.forEachSegment = function (a) {
    for (var b = this.arguments_, c = 0, f = 0, g = this.segments_.length; f < g; f++) {
        var h = this.segments_[f], k = goog.graphics.Path.segmentArgCounts_[h] * this.count_[f];
        a(h, b.slice(c, c + k));
        c += k
    }
};
...
goog.graphics.SvgGraphics.prototype.setElementFill = function (a, b) {
    var c = a.getElement();
    if (b instanceof goog.graphics.SolidFill)c.setAttribute("fill", b.getColor()), c.setAttribute("fill-opacity", b.getOpacity()); else if (b instanceof goog.graphics.LinearGradient) {
        var f = "lg-" + b.getX1() + "-" + b.getY1() + "-" + b.getX2() + "-" + b.getY2() + "-" + b.getColor1() + "-" + b.getColor2(), g = this.getDef(f);
        if (!g) {
            var g = this.createSvgElement_("linearGradient", {x1: b.getX1(), y1: b.getY1(), x2: b.getX2(), y2: b.getY2(), gradientUnits: "userSpaceOnUse"}),
                h = "stop-color:" + b.getColor1();
            goog.isNumber(b.getOpacity1()) && (h += ";stop-opacity:" + b.getOpacity1());
            h = this.createSvgElement_("stop", {offset: "0%", style: h});
            g.appendChild(h);
            h = "stop-color:" + b.getColor2();
            goog.isNumber(b.getOpacity2()) && (h += ";stop-opacity:" + b.getOpacity2());
            h = this.createSvgElement_("stop", {offset: "100%", style: h});
            g.appendChild(h);
            g = this.addDef(f, g)
        }
...
goog.graphics.SvgGraphics.prototype.createDom = function () {
    var a = this.createSvgElement_("svg", {width: this.width, height: this.height, overflow: "hidden"}), b = this.createSvgElement_("g");
...
goog.graphics.SvgGraphics.prototype.updateManualViewBox_ = function () {
    if (this.isInDocument() && (this.coordWidth || this.coordLeft || !this.coordTop)) {
        var a = this.getPixelSize();
        if (0 == a.width)this.getElement().style.visibility = "hidden"; else {
            this.getElement().style.visibility = "";
            var b = -this.coordLeft, c = -this.coordTop, f = a.width / this.coordWidth, a = a.height / this.coordHeight;
...
goog.graphics.SvgGraphics.prototype.getPixelSize = function () {
    ...
    var a = this.width, b = this.height, c = goog.isString(a) && -1 != a.indexOf("%"), f = goog.isString(b) && -1 != b.indexOf("%");
...
goog.graphics.SvgGraphics.prototype.drawTextOnLine = function (a, b, c, f, g, h, k, l, m, n) {
    ...
    var q = k.size;
    f = {"font-family": k.family, "font-size": q};
    var r = Math.round(0.85 * q), q = Math.round(c - q / 2 + r), r = b;
...
goog.graphics.SvgGraphics.prototype.drawPath = function (a, b, c, f) {
    a = this.createSvgElement_("path", {d: goog.graphics.SvgGraphics.getSvgPath(a)});
...
goog.graphics.VmlGroupElement.prototype.setSize = function (a, b) {
    var c = this.getElement(), f = c.style;
...
goog.graphics.VmlGraphics.prototype.setElementFill = function (a, b) {
    var c = a.getElement();
    this.removeFill(c);
    if (b instanceof goog.graphics.SolidFill)if ("transparent" == b.getColor())c.filled = !1; else if (1 != b.getOpacity()) {
        c.filled = !0;
        var f = this.createVmlElement("fill");
        f.opacity = Math.round(100 * b.getOpacity()) + "%";
        f.color = b.getColor();
        c.appendChild(f)
    } else c.filled = !0, c.fillcolor = b.getColor(); else if (b instanceof goog.graphics.LinearGradient) {
        c.filled = !0;
        ...
        var g = goog.math.angle(b.getX1(), b.getY1(), b.getX2(), b.getY2()), g = Math.round(goog.math.standardAngle(270 - g));
        f.angle = g;
        f.type = "gradient";
        c.appendChild(f)
    } else c.filled = !1;
...
goog.graphics.VmlGraphics.prototype.setElementStroke = function (a, b) {
    ...
    if (b) {
        c.stroked = !0;
        var f = b.getWidth(), f = goog.isString(f) && -1 == f.indexOf("px") ? parseFloat(f) : f * this.getPixelScaleX(), g = c.getElementsByTagName("stroke")[0];
...
goog.graphics.VmlGraphics.prototype.createDom = function () {
    ...
    var a =
        this.width, b = this.height, c = this.dom_.createDom("div", {style: "overflow:hidden;position:relative;width:" + goog.graphics.VmlGraphics.toCssSize(a) + ";height:" + goog.graphics.VmlGraphics.toCssSize(b)});
    this.setElementInternal(c);
    var f = this.createVmlElement("group"), g = f.style;
    g.position = "absolute";
    g.left = g.top = 0;
    g.width = this.width;
...
goog.graphics.VmlGraphics.prototype.handleContainerResize_ = function () {
    var a = goog.style.getSize(this.getElement()), b = this.canvasElement.getElement().style;
...
goog.graphics.VmlGraphics.prototype.drawTextOnLine = function (a, b, c, f, g, h, k, l, m, n) {
    var p = this.createFullSizeElement_("shape"), q = this.createVmlElement("path");
    ...
    a.forEachSegment(function (a, f) {
        switch (a) {
            ...
            case goog.graphics.Path.Segment.CLOSE:
                ...
                break;
            case goog.graphics.Path.Segment.ARCTO:
                var g = f[2] + f[3], h = goog.graphics.VmlGraphics.toSizeCoord(f[4] - goog.math.angleDx(g, f[0])), g = goog.graphics.VmlGraphics.toSizeCoord(f[5] - goog.math.angleDy(g, f[1])), k = goog.graphics.VmlGraphics.toSizeCoord(f[0]), l = goog.graphics.VmlGraphics.toSizeCoord(f[1]), m = Math.round(-65536 * f[2]), n = Math.round(-65536 * f[3]);
...
goog.graphics.CanvasTextElement = function (a, b, c, f, g, h, k, l, m, n) {
    var p = goog.dom.createDom(goog.dom.TagName.DIV, {style: "display:table;position:absolute;padding:0;margin:0;border:0"});
...
goog.graphics.CanvasTextElement.prototype.updateStyle_ = function () {
    var a = this.x1_, b = this.x2_, c = this.y1_, f = this.y2_, g = this.align_, h = this.font_, k = this.getElement().style, l = this.getGraphics().getPixelScaleX(), m = this.getGraphics().getPixelScaleY();
...
goog.graphics.CanvasGraphics.prototype.pushElementTransform = function (a) {
    ...
    b.save();
    a = a.getTransform();
    var c = a.getTranslateX(), f = a.getTranslateY();
    (c || f) && b.translate(c, f);
...
goog.graphics.CanvasGraphics.prototype.createDom = function () {
    var a = this.dom_.createDom("div", {style: "position:relative;overflow:hidden"});
    this.setElementInternal(a);
...
goog.graphics.CanvasGraphics.prototype.redraw = function () {
    if (this.preventRedraw_)this.needsRedraw_ = !0; else if (this.isInDocument()) {
        this.reset();
        if (this.coordWidth) {
            var a = this.getPixelSize();
...
goog.graphics.CanvasGraphics.prototype.drawElement = function (a) {
    if (!(a instanceof goog.graphics.CanvasTextElement)) {
        ...
        if (!a.getFill || !a.getStroke)a.draw(b); else {
            var c = a.getFill();
            if (c)if (c instanceof goog.graphics.SolidFill)0 != c.getOpacity() && (b.globalAlpha = c.getOpacity(), b.fillStyle = c.getColor(), a.draw(b), b.fill(), b.globalAlpha = 1); else {
...
goog.graphics.createGraphics = function (a, b, c, f, g) {
    a = goog.userAgent.IE && !goog.userAgent.isVersionOrHigher("9") ? new goog.graphics.VmlGraphics(a, b, c, f, g) : goog.userAgent.WEBKIT && (!goog.userAgent.isVersionOrHigher("420") || goog.userAgent.MOBILE) ? new goog.graphics.CanvasGraphics(a, b, c, f, g) : new goog.graphics.SvgGraphics(a, b, c, f, g);
    a.createDom();
    return a
};
goog.graphics.createSimpleGraphics = function (a, b, c, f, g) {
    return goog.userAgent.MAC && goog.userAgent.GECKO && !goog.userAgent.isVersionOrHigher("1.9a") ? (a = new goog.graphics.CanvasGraphics(a, b, c, f, g), a.createDom(), a) : goog.graphics.createGraphics(a, b, c, f, g)
};
...
goog.ui.Textarea.prototype.getHeight_ = function () {
    ...
    if (this.needsPaddingBorderFix_)b -= this.getPaddingBorderBoxHeight_(); else {
        if (!this.scrollHeightIncludesPadding_)var c = this.paddingBox_, b = b + (c.top + c.bottom);
...
goog.ui.Textarea.prototype.grow_ = function (a) {
    if (!this.isResizing_) {
        ...
        if (b.scrollHeight) {
            var f = !1, g = !1, h = this.getHeight_(), k = b.offsetHeight, l = this.getMinHeight_(), m = this.getMaxHeight_();
...
goog.ui.Textarea.prototype.shrink_ = function () {
    ...
    if (!this.isResizing_) {
        ...
        var b = a.scrollHeight;
        if (b) {
            var c = this.getHeight_(), f = this.getMinHeight_(), g = this.getMaxHeight_();
...
bay.whiteboard.Whiteboard = function () {
    ...
    for (var a = 0; a < bay.whiteboard.Whiteboard.toolGroups.length; a++) {
        var b = {id: bay.whiteboard.Whiteboard.toolGroups[a].id, order: bay.whiteboard.Whiteboard.toolGroups[a].order, desc: bay.whiteboard.Whiteboard.toolGroups[a].desc, hidden: !1};
    ...
    this.tool.tools = [];
    for (a = 0; a < bay.whiteboard.Whiteboard.tools.length; a++)toolProto = bay.whiteboard.Whiteboard.tools[a], tool = {id: toolProto.id, order: toolProto.order, desc: toolProto.desc, hidden: !1}, tool.action = toolProto.action, tool.toggleOn = toolProto.toggleOn, tool.toggleOff = toolProto.toggleOff, tool.onClick = toolProto.onClick, tool.onMove = toolProto.onMove, tool.group = toolProto.group, this.tool.tools.push(tool)
};
...
bay.whiteboard.Whiteboard.addGroup = function (a, b, c) {
    for (var f = 0; f < bay.whiteboard.Whiteboard.toolGroups.length; f++)if (bay.whiteboard.Whiteboard.toolGroups[f].id == a)return bay.whiteboard.Whiteboard.toolGroups[f];
    a = {id: a, order: b, desc: c};
...
bay.whiteboard.Whiteboard.addTool = function (a, b, c, f, g) {
    ...
    for (var h = 0; h < bay.whiteboard.Whiteboard.tools.length; h++)if (bay.whiteboard.Whiteboard.tools[h].id == a)return bay.whiteboard.Whiteboard.tools[h];
    a = {id: a, order: f, desc: g, group: b, action: c.action, toggleOn: c.toggleOn, toggleOff: c.toggleOff, onClick: c.onClick, onMove: c.onMove};
...
bay.whiteboard.Whiteboard.properties = {events: {onclick: !0, onwheel: !0, ondrag: !0, hover: !0, onrightclick: !0}, point: {size: 2, color: "black", width: 1, font: "Times", fontsize: 14}, hover: {width: 4, color: "blue", dist: 10}, current: {width: 1, color: "red"}, axes: {width: 0.5, color: "skyblue", font: "Times", fontsize: 11}};
...
bay.whiteboard.Whiteboard.prototype.redrawAll = function () {
    ...
    if (this.dragger && this.dragger.point && (point = this.dragger.point, point.exists)) {
        var b = new goog.graphics.Stroke(this.properties.axes.width,
            this.properties.axes.color), c = new goog.graphics.Font(this.properties.axes.fontsize, this.properties.axes.font), f = new goog.graphics.SolidFill(this.properties.axes.color), g = this.transform([point.x, point.y]);
...
bay.whiteboard.Whiteboard.prototype.getViewport = function () {
    return{left: this.area.minX, right: this.area.maxX, top: this.area.maxY, bottom: this.area.minY}
};
...
bay.whiteboard.Whiteboard.prototype.scale = function (a, b) {
    var c = this.reverseTransform(a), f = this.area.transformation.clone();
...
bay.whiteboard.Whiteboard.prototype.linkWebSocket = function (a) {
    this.ws_ = new goog.net.WebSocket;
    this.ws_.addEventListener(goog.net.WebSocket.EventType.OPENED, function (a) {
    }, !1, this);
    ...
    this.ws_.addEventListener(goog.net.WebSocket.EventType.CLOSED, function (a) {
        this.ws_.dispose()
    }, !1, this);
    this.ws_.addEventListener(goog.net.WebSocket.EventType.ERROR,
        function (a) {
            this.ws_.close()
        }, !1, this);
    this.ws_.open(a);
    ...
    this.collections.main.onChange = function (a) {
        b.ws_.isOpen() && b.ws_.send(this.getJson(a))
    };
    this.onBackground = function (a) {
        b.ws_.isOpen() && b.ws_.send(b.backgroundJson())
    }
};
...
bay.whiteboard.Whiteboard.prototype.onSetTransformation = function (a) {
    ...
    if (null != this.area.left && b[0] < this.area.left || null != this.area.right && b[2] > this.area.right || null != this.area.bottom && b[3] < this.area.bottom || null != this.area.top && b[1] > this.area.top)return this.area.transformation = a, this.area.reverseTransformation = this.area.transformation.createInverse(),
        !1;
    this.area.minX = b[0];
    this.area.minY = b[3];
    this.area.maxX = b[2];
...
bay.whiteboard.Whiteboard.prototype.findPoint = function (a) {
    for (var b = 0; b < a.length; b++)if (a[b].element instanceof bay.whiteboard.Point)return a[b].element;
    return null
};
bay.whiteboard.Whiteboard.prototype.pointAtEventPosition = function (a) {
    a = this.getConvertEventPos(a);
    var b = this.getHoverDist(), c = this.collections.main.getNeighbourList(a, b, !0, !0), f = this.findPoint(c);
    if (!f) {
        for (var g = 0; g < c.length && !(c[g].distance >= b); g++)for (var h = g + 1; h < c.length; h++) {
...
bay.whiteboard.Whiteboard.prototype.addDragListener = function () {
    if (this.properties.events.ondrag) {
        ...
        goog.events.listen(this.elements.drawElement, goog.events.EventType.MOUSEDOWN, function (b) {
            var c = this.getHoverDist(), c = this.collections.main.getNeighbourList(this.getConvertEventPos(b), c, !0, !0), c = this.findPoint(c);
...
bay.whiteboard.Whiteboard.prototype.TraceAll = function () {
    for (var a = this.collections.main.getElements(), b = 0; b < a.length; b++)if (a[b] && (a[b].trace && a[b].exists) && a[b].getTrace) {
        var c = a[b].getTrace();
        c.exists = !0;
...
bay.whiteboard.Whiteboard.prototype.addButtons = function () {
    var a = this, b = function (b, c, f) {
        var g = goog.style.getSize(a.elements.toolbarElement), n = new goog.ui.Button;
    ...
    a.tool.tools.sort(c);
    for (f = 0; f < a.tool.tools.length; f++)if (tool = a.tool.tools[f], !tool.hidden)if (null == tool.group)b(tool.id, tool); else for (c = 0; c < a.tool.groups.length; c++)a.tool.groups[c].id ==
...
bay.whiteboard.Whiteboard.prototype.buttonAction = function (a, b) {
    if (a.action)this.tool.current && this.tool.current.toggleOff && (this.tool.current.toggleOff(this), this.tool.current = null), a.action(this, b); else {
...
bay.whiteboard.Whiteboard.prototype.showToolBox = function (a) {
    var b = goog.style.getPageOffset(a.button.getElement()), c = goog.style.getSize(a.button.getElement());
    b.x += c.width;
...
bay.whiteboard.Whiteboard.prototype.showInfoDialog = function (a) {
    var b = this.getHoverDist(), b = this.collections.main.getNeighbourList(this.getConvertEventPos(a), b, !0, !0);
...
bay.whiteboard.Whiteboard.prototype.drawCoordinates = function () {
    var a = new goog.graphics.Stroke(this.properties.axes.width, this.properties.axes.color), b = new goog.graphics.Font(this.properties.axes.fontsize, this.properties.axes.font), c = new goog.graphics.SolidFill(this.properties.axes.color), f = this.transform([this.area.minX, 0, this.area.maxX, 0, 0, this.area.minY, 0, this.area.maxY]), g = new goog.graphics.Path;
    g.moveTo(f[0], f[1]);
    g.lineTo(f[2], f[3]);
    ...
    f = 1;
    if (1 > g)for (; 1 > g * f;)f *= 10; else for (; 10 < g * f;)f /= 10;
    for (var g = Math.round(g * f) / f, h = this.area.minX, h = Math.floor(h / g) * g; h < this.area.maxX;) {
        var f = this.transform([h, -g / 10, h, g / 10, h, 0]), k = new goog.graphics.Path;
        k.moveTo(f[0], f[1]);
        k.lineTo(f[2], f[3]);
...
bay.whiteboard.geometry.Point_2l.prototype.recalc = function () {
    if (!this.obj1 || !this.obj2 || !this.obj1.exists || !this.obj2.exists)this.exists = !1; else {
        var a = this.obj1.direction.x, b = this.obj1.direction.y, c = this.obj2.direction.x, f = this.obj2.direction.y, g = c * b - f * a;
...
bay.whiteboard.geometry.Point_2c.prototype.recalc = function () {
    if (!this.obj1 || !this.obj2 || !this.obj1.exists || !this.obj2.exists)this.exists = !1; else {
        var a = this.obj1.radius, b = this.obj2.radius, c = this.obj2.centerPoint.x - this.obj1.centerPoint.x, f = this.obj2.centerPoint.y - this.obj1.centerPoint.y, g = c * c + f * f, c = Math.sqrt(c * c + f * f);
...
bay.whiteboard.geometry.Point_lc.prototype.recalc = function () {
    if (!this.obj1 || !this.obj2 || !this.obj1.exists || !this.obj2.exists)this.exists = !1; else if (this.obj1.distanceTo(this.obj2.centerPoint) > this.obj2.radius)this.exists = !1; else {
        var a = this.obj1.direction.x * this.obj1.direction.x + this.obj1.direction.y * this.obj1.direction.y, b = 2 * this.obj1.direction.x * (this.obj1.startPoint.x - this.obj2.centerPoint.x) + 2 * this.obj1.direction.y * (this.obj1.startPoint.y - this.obj2.centerPoint.y), c = Math.sqrt(b * b - 4 * a * ((this.obj1.startPoint.x -
...
bay.whiteboard.Whiteboard.addTool("point", "geometry", {toggleOn: function (a) {
...
bay.whiteboard.geometry.Line.prototype.getMinAndMaxParamValue = function (a) {
    var b = -Number.MAX_VALUE, c = Number.MAX_VALUE;
    if (0 != this.direction.x && (b = (a.minX - this.startPoint.x) / this.direction.x, c = (a.maxX - this.startPoint.x) / this.direction.x, b > c))var f = c, c = b, b = f;
    var g = -Number.MAX_VALUE, h = Number.MAX_VALUE;
    ...
    return b < h && c > g ? (a = g, b > g && (a = b), b = h, c < h && (b = c), {min: a, max: b}) : null
};
bay.whiteboard.geometry.Line.prototype.draw = function (a) {
    if (this.exists) {
        var b = this.getMinAndMaxParamValue(a.area);
        if (b) {
            var c = a.transform([this.startPoint.x + b.min * this.direction.x, this.startPoint.y + b.min * this.direction.y, this.startPoint.x + b.max * this.direction.x, this.startPoint.y + b.max * this.direction.y]), b = new goog.graphics.Path;
            b.moveTo(c[0], c[1]);
            b.lineTo(c[2], c[3]);
...
bay.whiteboard.Whiteboard.properties.line = {width: 1, color: "Black"};
...
bay.whiteboard.Whiteboard.addTool("ruler", "geometry", {toggleOn: function (a) {
...
bay.whiteboard.Whiteboard.addTool("segment", "geometry", {toggleOn: function (a) {
    goog.dom.classes.add(a.elements.drawElement, "bwb_rulerCursor");
    a.tool.current.ruler = {}
}, toggleOff: function (a) {
    a.clearCurrentTool("bwb_rulerCursor", "ruler")
...
bay.whiteboard.geometry.Circle.prototype.closestPoint = function (a, b) {
    var c = new bay.whiteboard.Vector(a, b), c = new bay.whiteboard.Vector(c.x - this.centerPoint.x, c.y - this.centerPoint.y);
...
bay.whiteboard.Whiteboard.properties.circle = {width: 1, color: "Black"};
...
bay.whiteboard.Whiteboard.addTool("compass", "geometry", {toggleOn: function (a) {
...
bay.whiteboard.pencil.Curve = function (a) {
    ...
    a.dependant.push(this);
    this.points = [];
    this.pos = {left: a.x, right: a.x, top: a.y, bottom: a.y};
    this.exists = !0
};
...
bay.whiteboard.pencil.Curve.prototype.draw = function (a) {
    if (this.exists) {
        var b = a.transform([this.startPoint.x, this.startPoint.y]), c = new goog.graphics.Path;
        c.moveTo(b[0], b[1]);
...
bay.whiteboard.Whiteboard.properties.curve = {width: 3, color: "Gray"};
bay.whiteboard.pencil.Curve.prototype.toJson = function (a, b) {
    str = "{" + this.jsonHeader(b) + ', "type": "PencilCurve", "p0": ' + a.indexOf(this.startPoint);
...
bay.whiteboard.pencil.Curve.fromJson = function (a, b) {
    for (var c = new bay.whiteboard.pencil.Curve(b[a.p0]), f = 0; "undefined" != typeof a["x" + f];)c.points.push(new bay.whiteboard.Vector(a["x" + f], a["y" + f])), f++;
    c.restoreFromJson(a);
    return c
};
...
bay.whiteboard.pencil.Curve.prototype.acceptData = function (a) {
    ...
    this.points = [];
    for (var b = 0; "undefined" != typeof a["x" + b];)this.points.push(new bay.whiteboard.Vector(a["x" + b], a["y" + b])), b++;
    this.recalc()
};
bay.whiteboard.Whiteboard.addTool("curve", "pencil", {toggleOn: function (a) {
    ...
    if (a.tool.current.curve.start)a.tool.current.toggleOff(a); else {
        var c = a.pointAtEventPosition(b), c = new bay.whiteboard.pencil.Curve(c);
...
bay.whiteboard.Whiteboard.properties.freeline = {width: 3, color: "Gray"};
...
bay.whiteboard.Whiteboard.addTool("freeline", "pencil", {toggleOn: function (a) {
...
bay.whiteboard.pencil.Rectangle = function (a, b) {
    ...
    a.dependant.push(this);
    b.dependant.push(this);
    this.pos = {left: null, right: null, top: null, bottom: null};
    this.noLabel = !0;
    this.recalc()
};
...
bay.whiteboard.pencil.Rectangle.prototype.closestPoint = function (a, b) {
    var c = new bay.whiteboard.Vector(a, b), f = null, g = null;
    if (c.x <= this.pos.left)f = "left", g = c.y < this.pos.bottom ? 0 : c.y >= this.pos.top ? 1 : (c.y - this.pos.bottom) / (this.pos.top - this.pos.bottom); else if (c.x >= this.pos.right)f = "right", g = c.y < this.pos.bottom ? 0 : c.y >= this.pos.top ? 1 : (c.y - this.pos.bottom) / (this.pos.top - this.pos.bottom); else if (c.y <= this.pos.bottom)f = "bottom", g = (c.x - this.pos.left) / (this.pos.right - this.pos.left); else if (c.y >= this.pos.top)f =
        "top", g = (c.x - this.pos.left) / (this.pos.right - this.pos.left); else {
        var h = this.pos.top - c.y, f = "top", g = (c.x - this.pos.left) / (this.pos.right - this.pos.left);
...
bay.whiteboard.pencil.Rectangle.prototype.draw = function (a) {
    if (this.exists) {
        var b = a.transform([this.pos.left, this.pos.bottom, this.pos.right, this.pos.top]), c = new goog.graphics.Path;
        c.moveTo(b[0], b[1]);
        c.lineTo(b[0], b[3]);
        c.lineTo(b[2], b[3]);
        c.lineTo(b[2], b[1]);
...
bay.whiteboard.Whiteboard.properties.rectangle = {width: 3, color: "Grey"};
...
bay.whiteboard.Whiteboard.addTool("rectangle", "pencil", {toggleOn: function (a) {
...
bay.whiteboard.Whiteboard.properties.pencilcircle = {width: 3, color: "Grey"};
...
bay.whiteboard.Whiteboard.addTool("pencilcircle", "pencil", {toggleOn: function (a) {
...
bay.whiteboard.pencil.Text.prototype.width = function (a, b) {
    var c = goog.dom.createDom("div", {style: "position: absolute; float: left; white-space: nowrap; display: hidden; font: " + (b + "px " + a) + ";"}, this.label);
...
bay.whiteboard.pencil.Text.prototype.draw = function (a) {
    if (this.exists) {
        var b = a.transform([this.rectangle.pos.left, this.rectangle.pos.top]), c = (this.rectangle.pos.right - this.rectangle.pos.left) * a.area.transformation.getScaleX(), f = (this.rectangle.pos.bottom - this.rectangle.pos.top) * a.area.transformation.getScaleY(), g = new goog.graphics.Font(f, a.properties.text.font), g = this.width(a.properties.text.font, f), h = f;
        g > c && (h = f * c / g);
...
bay.whiteboard.Whiteboard.properties.text = {width: 1, color: "DarkBlue", font: "Times"};
...
bay.whiteboard.Whiteboard.addTool("text", "pencil", {toggleOn: function (a) {
...
bay.whiteboard.Whiteboard.properties.underline = {thickness: 15, color: "Magenta", opacity: 0.3};
...
bay.whiteboard.Whiteboard.addTool("underline", "pencil", {toggleOn: function (a) {
...
bay.whiteboard.pencil.Arrow.prototype.draw = function (a) {
    if (this.exists) {
        ...
        if (b && (0 < b.max && 1 > b.min) && (b = Math.sqrt(this.direction.x * this.direction.x + this.direction.y * this.direction.y), 0 != b)) {
            ...
            b.lineTo(c[0], c[1]);
            if (this.current)c =
                new goog.graphics.Stroke(a.properties.current.width, a.properties.current.color), a.graphics.drawPath(b, c, null); else {
                if (this.hover) {
...
bay.whiteboard.Whiteboard.properties.arrow = {width: 1, thickness: 3, color: "Gray", opacity: 0.3};
...
bay.whiteboard.Whiteboard.addTool("arrow", "pencil", {toggleOn: function (a) {
...
bay.whiteboard.pencil.Pointer.prototype.draw = function (a) {
    if (this.exists && this.point.x >= a.area.minX && this.point.x <= a.area.maxX && this.point.y >= a.area.minY && this.point.y <= a.area.maxY) {
        var b = a.properties.pointer.radius * (this.age % 500 / 500) * a.area.transformation.getScaleX(), c = [];
        c[0] = this.point.x;
        c[1] = this.point.y;
        ...
        var c = a.transform(c), f = new goog.graphics.Path;
        f.moveTo(c[2], c[3]);
        f.lineTo(c[4], c[5]);
...
bay.whiteboard.Whiteboard.properties.pointer = {width: 1, radius: 30, color: "Magenta", age: 1E4};
...
bay.whiteboard.Whiteboard.addTool("pointer", "pencil", {toggleOn: function (a) {
...
bay.whiteboard.art.ClipArt.prototype.draw = function (a) {
    if (this.exists) {
        ...
        g.lineTo(f[0],
            f[1]);
        if (this.current)b = new goog.graphics.Stroke(a.properties.current.width, a.properties.current.color), a.graphics.drawPath(g, b, null); else {
            if (this.hover) {
...
bay.whiteboard.Whiteboard.addTool("clipart", "pencil", {toggleOn: function (a) {

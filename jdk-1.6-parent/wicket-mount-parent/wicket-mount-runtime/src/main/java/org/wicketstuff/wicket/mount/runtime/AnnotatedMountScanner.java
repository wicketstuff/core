 ...
 */
package org.wicketstuff.wicket.mount.runtime;
...
import org.wicketstuff.wicket.mount.core.AnnotatedMountList;
import org.wicketstuff.wicket.mount.core.annotation.MountPath;
import org.wicketstuff.wicket.mount.runtime.config.MatchingResources;
...
public class AnnotatedMountScanner
{
	...
	private void scanClass(Class<? extends Page> pageClass, AnnotatedMountList list)
	{
		...
		if (mountPath == null)
		{
			mountPath = pageClass.getPackage().getAnnotation(MountPath.class);
			if (mountPath == null)
			{
				return;
			}
		...
		if (path.endsWith("/*"))
		{
			path = path.substring(0, path.length() - 1) + getDefaultMountPath(pageClass);
		}

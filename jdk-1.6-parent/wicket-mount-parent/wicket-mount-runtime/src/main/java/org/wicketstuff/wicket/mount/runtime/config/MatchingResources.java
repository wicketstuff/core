...
{
	private final static Logger logger = LoggerFactory.getLogger(MatchingResources.class);
	 ...
	 * @param includeSubclasses if true, this will also return classes whose superclass has the specified
	 *                          annotation
	                                          ...
	                                          boolean includeSubclasses)
	{
		...
		for (Resource r : resources)
		{
			...
			if (types.contains(annotation.getName()) ||
					anyPackageHas(getClass(anno.getClassName()), annotation) ||
					(includeSubclasses && anySuperHas(getClass(anno.getSuperClassName()), annotation)))
			{
				if (anno.getClassName().endsWith("package-info"))
				{
					logger.debug("Skip package-info processing of class {}", anno.getClassName());
				} else
				{
	...
	private boolean anyPackageHas(Class<?> clz, Class<? extends Annotation> annotation)
	{
		return clz != null && clz.getPackage().isAnnotationPresent(annotation);
	}

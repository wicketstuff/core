...
public class HomePage extends WicketExamplePage {
	...
	public HomePage() {
		...
		map.add(new ClickListener() {
			...
			@Override
			protected void onClick(final AjaxRequestTarget target, final GLatLng latLng) {
				if (latLng != null) {
					...
					final MyMarker marker = new MyMarker(new GMarkerOptions(map, latLng).draggable(true)) {
						...
						GEventHandler getDragendHandler() {
							return new GEventHandler() {
								...
								@Override
								public void onEvent(final AjaxRequestTarget eventTarget) {
					...
					rv.removeAll();
					for (final GOverlay myMarker : map.getOverlays()) {
						final GOverlayPanel label =
						        new GOverlayPanel(myMarker.getId(), new CompoundPropertyModel<MyMarker>(
						                (MyMarker) myMarker));
	...
	private static class GOverlayPanel extends Panel {
		...
		public GOverlayPanel(final String id, final IModel<MyMarker> model) {
			super(id, model);
			...
			dragendLabel.add(new AjaxEventBehavior("onclick") {
				...
				protected void onEvent(final AjaxRequestTarget target) {
					final MyMarker overlay = model.getObject();
			...
			dblclickLabel.add(new AjaxEventBehavior("onclick") {
				...
				@Override
				protected void onEvent(final AjaxRequestTarget target) {
					final MyMarker overlay = ((MyMarker) GOverlayPanel.this.getDefaultModelObject());
	...
	private static abstract class MyMarker extends GMarker {
		...
		public MyMarker(final GMarkerOptions options) {
			super(options);
		}

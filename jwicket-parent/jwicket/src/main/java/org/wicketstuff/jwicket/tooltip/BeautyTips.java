...
import org.wicketstuff.jwicket.tooltip.res.beutytips.Anchor;
...
public class BeautyTips extends AbstractToolTip {
	...
	IHeaderContributor getHeadercontributor() {
		return new IHeaderContributor() {
			...
			public void renderHead(final IHeaderResponse response) {
				...
				response.renderJavascriptReference(new JavascriptResourceReference(Anchor.class, "jquery.hoverIntent.minified.js"));
				
				response.renderJavascriptReference(new JavascriptResourceReference(Anchor.class, "jquery.bgiframe.min.js"));
				if (ieVersion > 0)
					response.renderJavascriptReference(new JavascriptResourceReference(Anchor.class, "excanvas.compiled.js"));
				response.renderJavascriptReference(new JavascriptResourceReference(Anchor.class, "jquery.bt.min.js"));
				response.renderJavascriptReference(new JavascriptResourceReference(Anchor.class, "jquery.easing.1.3.js"));
	...
	String getJavaScript() {
		...
		for (Component component : components) {
			...
			if (cssStyles != null && cssStyles.trim().length() > 0) {
				if (!firstOption)
					builder.append(",");
				else
					firstOption = false;
			...
			if (activeClass != null && activeClass.trim().length() > 0) {
				if (!firstOption)
					builder.append(",");
				else
					firstOption = false;

 ...
 *
 * @param <T> the type of the model object (can be non serializable)
...
public abstract class AbstractAutoCompleteTextField<T> extends TextField<T> implements IJQueryWidget, IAutoCompleteListener // NOSONAR
{
	 ...
	 * As it is a model it can be a {@link org.apache.wicket.model.LoadableDetachableModel}
	 * So that choices are not serialised with component.
	 */
	...
	/**
	 * Call {@link #getChoicesModel(String)} and cache the result<br>
	 * Internal use only
	 *
	 * @param input String that represent the query
	 * @return the list of choices
	 */
	private IModel<List<T>> internalGetChoices(String input)
	{
		this.choices = this.getChoicesModel(input);
		return this.choices;
	}
	 ...
	 *
	 * @param input String that represent the query
	 * @return IModel with the list of choices
	 */
	protected abstract IModel<List<T>> getChoicesModel(String input);
	 ...
	 * @return {@link JQueryAbstractTemplateBehavior}
	 */

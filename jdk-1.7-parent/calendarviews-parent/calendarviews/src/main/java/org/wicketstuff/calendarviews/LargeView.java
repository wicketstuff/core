...
public class LargeView extends FullWeekCalendarView
{
	...
	private static final VersionDescriptor JS_LIB_VERSION_DESCRIPTOR = VersionDescriptor.alwaysLatestOfVersion(
		Library.PROTOTYPE, 1, 6);
	private static final long serialVersionUID = 1L;
	private static final Logger LOGGER = LoggerFactory.getLogger(LargeView.class);

	public static final PackageResourceReference JS = new PackageResourceReference(LargeView.class,
		"LargeView.js");
	...
	protected final ListView<IEvent> createEventListView(String id,
		final IModel<DateMidnight> dateModel, final int cellsLeftInRow, IModel<List<IEvent>> model)
	{
	...
	private class LargeGridView extends GridView<DateMidnight>
	{
		...
		protected void populateItem(final Item<DateMidnight> item)
		{
			...
			Label dateHeader = new Label("date", new PropertyModel<Integer>(item.getModel(),
				"dayOfMonth"));
	...
	protected void onMoreLinkClicked(AjaxRequestTarget target, IModel<DateMidnight> model,
	...
	protected PageCreator getDetailModalPageCreator(final IModel<DateMidnight> model,

...
public class GMapTest
{
	...
	public void testIfBoundJSIsRenderedIfBoundsPropertyIsSet()
	{
		WicketTester tester = new WicketTester();
		GMap map = new GMap("someId");
		map.setBounds(new GLatLngBounds(new GLatLng(43.8864731, 4.739279), new GLatLng(43.9966409,
			4.927226)));
		tester.startComponentInPage(map);
		Assert.assertTrue(
			"If the property bounds is not set, it shoudn'd be render the JS-Methode",
			map.getJSinit()
				.contains(
					"fitBounds(new google.maps.LatLngBounds(new google.maps.LatLng(43.8864731, 4.739279, false), new google.maps.LatLng(43.9966409, 4.927226, false)))"));
	}

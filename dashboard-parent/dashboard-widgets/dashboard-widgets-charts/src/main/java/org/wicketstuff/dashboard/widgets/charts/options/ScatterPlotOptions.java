...
public class ScatterPlotOptions extends ShowcaseOptions {
	...
	private static List<Coordinate<Number, Number>> fromJSON(String jsonArray) {
		List<Coordinate<Number, Number>> seriesData = new ArrayList<Coordinate<Number, Number>>();

		// reformat into xValue,yValue;xValue,yValue
	...
	public ScatterPlotOptions() {
		...
		setTitle(new Title("Height Versus Weight of 507 Individuals by Gender"));
		setSubtitle(new Title("Source: Heinz  2003"));

		Axis xAxis = new Axis();
		xAxis.setTitle(new Title("Height (cm)"));
		xAxis.setLabels(new Labels().setStyle(new CssStyle()));
		...
		yAxis.setLabels(new Labels().setStyle(new CssStyle()));
		setyAxis(yAxis);

		Function function = new Function();
		function.setFunction("return '' + this.x + ' cm, ' + this.y + ' kg';");
		...
		series1.setColor(new RgbaColor(223, 83, 83, 0.5f));
		series1.setName("Female");
		series1.setData(ScatterPlotOptions
			.fromJSON(ScatterPlotOptions.FEMALE_DATA)
		);
		addSeries(series1);
		...
		series2.setData(ScatterPlotOptions
	...
	public String getLabel() {
		return super.getLabel() + "Scatter plot";
	}
}
...
public abstract class EditableGridActionsPanel<T> extends Panel {
    ...
    public EditableGridActionsPanel(final String id, final Item<ICellPopulator<T>> cellItem) {
        super(id);

        @SuppressWarnings("unchecked") final Item<T> rowItem = cellItem.findParent(Item.class);
...
//                send(getPage(), Broadcast.BREADTH, new GridOperationData<T>(OperationType.DELETE, (T) rowItem.getDefaultModelObject(), eventTarget));
    ...
    private boolean isThisRowBeingEdited(final Item<T> rowItem) {

...
public class Chart extends WebComponent
{
	...
	private CharSequence render(IChartData data)
	{
		...
		for (double[] set : data.getData())
		{
			...
			else
			{
				...
				{
					back.append(encoding.convert(value, data.getMax())).append(
						encoding.getValueSeparator());
				}
		...
		if (back.substring(back.length() - encoding.getSetSeparator().length(), back.length())
			.equals(encoding.getSetSeparator()))
		{
	...
	private CharSequence renderStyles(IChartAxis[] axes)
	{
		...
		for (int i = 0; i < axes.length; i++)
		{
			if (axes[i] == null || axes[i].getColor() == null || axes[i].getFontSize() < 0 ||
				axes[i].getAlignment() == null)
			{
				continue;
			}

...
public class SecureLinkTest extends WaspAbstractTestBase {
    ...
    {
        // step zero, login and you will not see the PageA link (it has no authorization, the default render check
        ...
        // step two, show the secure home page with a not clickable link to PageA (e.g. not a href)
        Map authorized = new HashMap();
        ...
        authorized.clear();
        authorized.put(HomePage.class, application.getActionFactory().getAction("access render enable"));
        authorized.put(PageA.class, application.getActionFactory().getAction("render enable"));
        login(authorized);
        ...
        logoff(authorized);
        authorized.clear();
        // step four, show the secure home page with a clickable link and click the link that is not enabled.
        linkcheck.setUseAlternativeRenderCheck(false);
        authorized.put(HomePage.class, application.getActionFactory().getAction("access render enable"));
        authorized.put(PageA.class, application.getActionFactory().getAction("access render"));
        login(authorized);
        mock.setupRequestAndResponse();
        mock.processRequestCycle();
        mock.assertRenderedPage(getHomePage());

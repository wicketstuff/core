...
import com.sas.ui.wicket.modal.ModalContentPanel;
...
/**
 * Class Name : ModalFormPanel
 * Diagram    : Wicket Generic Modal Windows
 * Project    : Echobase framework
 * Type       : concrete
 * Base class for all modal content panels that contain a form.
 * 
 * @author Chris Colman
 */
public 
class ModalFormPanel extends ModalContentPanel
{
// -[KeepWithinClass]-
...
/**
 * Adds the button controls to the form. By default OK and Cancel buttons are added but
 * this method can be overridden to add a different button set like YES, NO etc.,
 */
...
/**
 * Called after form submission with successful validation to update the domain model
 * with data from the form components.
 */
 ...
 * Calll form.add to add components, not just add.
 */
...
/**
 * Creates a Form object. Can be overridden in derived classes to create a form object
 * that extends Form.
 */
...
/**
 * Constructs the object passing in a IWindowClosedListener (which can be null if parent
 * is not interested in being notified when the child form is closed).
 */

 ...
 * Note that although the current implementation is required to work with
 * {@link ISecureModel}s, it is recommended to implement SwarmModel instead as
 * it allows you more control to specify a name for the permission. If you do
 * use a regular ISecureModel we fall back to the toString method.
 * 
 * @author marrink
...
public interface SwarmModel<T> extends ISecureModel<T>
{
	 ...
	 * used as the name of a {@link DataPermission} by
	 * {@link SwarmStrategy#isModelAuthorized(ISecureModel, Component, org.apache.wicket.security.actions.WaspAction)}
	 * And is therefore required to remain consistent or each invocation
	 * provided the same component is used.
	 * 
	 * @param component

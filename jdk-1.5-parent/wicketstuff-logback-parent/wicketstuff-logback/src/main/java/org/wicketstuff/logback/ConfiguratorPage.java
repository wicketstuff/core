...
/**
 * A page that shows all registered loggers in a drop-down list
 * and allows changing the effective level per logger.
 * Useful for online debugging.
 */
public class ConfiguratorPage extends WebPage {
    ...
    private static final List<Level> LEVELS = 
            Arrays.asList(Level.WARN, Level.DEBUG, Level.TRACE, Level.INFO, Level.ERROR, Level.ALL);
    ...
    public ConfiguratorPage(final PageParameters parameters) {
        ...
        final DropDownChoice<Logger> loggersDDC = new DropDownChoice<Logger>("loggers", Model.of((Logger)null), loggers);
        ...
        final DropDownChoice<Level> levelsDDC = new DropDownChoice<Level>("levels", Model.of((Level)null), LEVELS);
        ...
        levelsDDC.add(new AjaxFormComponentUpdatingBehavior("onchange") {
            ...
            protected void onUpdate(AjaxRequestTarget target) {
                ...
                logger.debug("Logger '{}' now have level '{}'.", log.getName(), level);

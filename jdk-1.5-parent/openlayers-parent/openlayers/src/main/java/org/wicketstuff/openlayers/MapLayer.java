...
public class MapLayer implements Serializable {
	...
	public void addFeature(String uniqKey, Feature feature,
			String baseFeatureUniqKey, String color) {
		FeatureStyle baseFeatureStyle = featureStyleList
				.get(baseFeatureUniqKey);
	...
	public void addFeature(String uniqKey, Feature feature,
		...
		featureList.put(uniqKey,
				new Feature2FeatureStyle(feature, featureStyle));
		if (map != null) {
			if (featureStyle != null) {
	...
	public AjaxOpenLayersMap getMapPanel(final String id,
			final List<Layer> layers, final HashMap<String, String> options) {
		if (map == null) {
			if (init_location != null) {
				if (center == null) {
					setCenter(init_location.getLongitude(), init_location
							.getLatitude());
				}

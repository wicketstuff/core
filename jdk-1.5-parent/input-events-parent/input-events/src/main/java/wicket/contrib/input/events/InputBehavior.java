...
public class InputBehavior extends Behavior
{
	...
	private final TextTemplate shortcutJsAutoHookLink = new PackageTextTemplate(
		InputBehavior.class, "wicket-contrib-input-behavior-autohook-link.js");
	...
	/**
	 * if using auto hook be sure to add this behavior last, otherwise it might not pickup the
	 * event.. Also it will only hook up to the last event if more are present (use other
	 * constructor to specify manually)
	 * 
	 * 
	 * Note on keyCombo
	 * 
	 * The shortcut keys should be specified in this format ... Modifier[+Modifier..]+Key
	 * 
	 * Meaning that you should specify in this order, modifier keys first like 'ctrl' and then
	 * normal keys like 'a'
	 * 
	 * @param keyCombo
	 * @param autoHook
	...
	/**
	 * If this is set to true, keyboard capture will be disabled in input and textarea fields. If
	 * these elements have focus, the keyboard shortcut will not work. This is very useful for
	 * single key shortcuts. Default: false
	 * 
	 * @return
	 */
	...
	/**
	 * target - DOM Node The element that should be watched for the keyboard event. Default :
	 * document
	 * 
	 * @return
	 */
